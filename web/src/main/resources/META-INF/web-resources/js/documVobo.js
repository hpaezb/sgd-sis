function fn_inicializaBusqDocsVoBo(){
    var noForm='#buscarDocumentoVoBoBean';
    jQuery(noForm).find("#fechaFiltro").showDatePicker({defaultOpcionSelected: 9});        
    jQuery(noForm).find('#nuDoc').change(function() {
        $(this).val(replicate($(this).val(), 6));
    });
    fn_validarTipoBusquedaDocsVoBo(noForm,'0');
    pnumFilaSelect=0;
}
function fn_validarTipoBusquedaDocsVoBo(noForm,tiBusq){
        ...
        if(validarBusqDocsVoBo(noForm)){
            ...
            if(pEsIncluyeFiltro){//filtro
                if(validarFiltroDocsVoBo(noForm)){
                    fn_submitAjaxFormBusqDocsVoBo(noForm);
                }
            }else{
                fn_submitAjaxFormBusqDocsVoBo(noForm);
            }
...
function fn_submitAjaxFormBusqDocsVoBo(noForm){
    ajaxCall("/srDocumentoVoBo.do?accion=goInicio", jQuery(noForm).serialize(), function(data) {
...
function validarBusqDocsVoBo(noForm){
    ...
    var vdeAsunto=jQuery(noForm).find('#asunto').val();
    
    if(!!vRetorno){
        if(!!vdeAsunto){
            //validar caracteres especiales
            vdeAsunto=jQuery(noForm).find('#asunto').val(fn_getCleanTextLenGreathree(fn_getCleanTextExpReg(vdeAsunto))).val();
            if(allTrim(vdeAsunto).length >= 0 && allTrim(vdeAsunto).length <= 3){  //El texto es entre 1 y 3 caracteres
                vRetorno = 0;
...
function editarDocumentoVoBoButtonToolBar() {
    var pnuAnn = jQuery('#txtpnuAnn').val();
    var pnuEmi = jQuery('#txtpnuEmi').val();
    editarDocumentoVoBo(pnuAnn,pnuEmi);
}

function editarDocumentoVoBo(pnuAnn,pnuEmi){
    var noForm='#documentoVoBoBean';
    if (!!pnuAnn&&!!pnuEmi) {
        var p = new Array();
        p[0] = "pnuAnn=" + pnuAnn;
        p[1] = "pnuEmi=" + pnuEmi;
        ajaxCall("/srDocumentoVoBo.do?accion=goEditDocumentoVoBo", p.join("&"), function(data) {
...
function fn_eventEstDocVoBo(sEstadoDocAdm,noForm) {
    ...
    if (valDoc === "SI") {
        fn_cargaDocVoBoFirmaApplet(vnoDoc,noForm, function(data) {
            ...
            if(resulCarga.error==="0"){
                jQuery(noForm).find('#nuSecuenciaFirma').val(resulCarga.message);
                ajaxCall("/srDocumentoVoBo.do?accion=goChangeToEnviado", $(noForm).serialize(), function(data) {
                    fn_rptaChangeToEnviadoDocVoBo(data,noForm);
                },'json', false, false, "POST");
            }
...
function fn_regresarDocsVoBoLs(pclickBtn) {
    var noForm='#documentoVoBoBean';
    if (pclickBtn === "1") {
        var vEsDocEmi = jQuery(noForm).find('#esDocEmi').val();
        if(vEsDocEmi==="2" || vEsDocEmi==="7" || vEsDocEmi==="0"){
            if(fn_verificarCamposDocVoBoObs(noForm)){
                var cadenaJson=fn_buildSendJsontoServerDocVistoBueno(noForm);
                if(fn_existeCambioEnDocVoBoBean(new Function('return ' + cadenaJson)())){
                    bootbox.dialog({
...
function fn_continueRegresarDocVoBoLs(){
    jQuery('#divNewEmiDocumVoBo').toggle();                                
    jQuery('#divBusqDocumentoVoBo').toggle(); 
    fn_validarTipoBusquedaDocsVoBo('#buscarDocumentoVoBoBean',jQuery('#buscarDocumentoVoBoBean').find('#tipBusqueda').val());
...
function fn_buscaReferenciaOrigenBusqDocVoBo() {
    ...
    p[1] = "pnuAnn=" + jQuery('#buscarDocumentoVoBoBean').find('#nuAnn').val();               
...
function fn_buscaReferenciaOrigenBusqDocVoBoConsul() {
    var valor="C";
    var p = new Array();
    p[0] = "accion=goBuscaReferenciaOrigen";
    p[1] = "pnuAnn=" + jQuery('#buscarDocumentoVoBoBean').find('#nuAnn').val(); 
...
function fn_buscaElaboradoPorDocVobo() {
...
function fn_buscaElaboradoPorDocVoboConsul() {
    var valor="C";
    var p = new Array();
    p[0] = "accion=goBuscaElaboradoPor";
    p[1] = "pconsulta=" +valor;
    ajaxCall("/srDocumentoVoBo.do", p.join("&"), function(data) {
        fn_rptaBuscaElaboradoPorDocVoboConsul(data);
    },'text', false, false, "POST");
...
function fn_showObsEmpPerVoBo(coDep,coEmp){

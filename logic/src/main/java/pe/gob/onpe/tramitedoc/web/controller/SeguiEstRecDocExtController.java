 ...
 */
@Controller
@RequestMapping("/{version}/srSeguiEstRecDocExt.do")
public class SeguiEstRecDocExtController {
    @Autowired
    ...
    public String goInicio(HttpServletRequest request, Model model) {
        ...
        if(codDependencia!=null&&codDependencia.trim().length()>0){
            ...
            buscarDocExtRecSeguiEstBean.setCoDepEmi(usuarioConfigBean.getCoDep());
            buscarDocExtRecSeguiEstBean.setDeDepEmi(commonQryService.getDependenciaxCoDependencia(usuarioConfigBean.getCoDep()).getDeDependencia());
            ...
            return "/segEstRecDocExt/consultaRegDocExt";    
        }else{
...
//        buscarDocumentoSeguiEstRecBean.setTiAcceso(tipAcceso);
        ...
        if (mensaje.equals("OK")) {
            return "/segEstRecDocExt/tblDocsExterno";
        } else {
    ...
    public String goDetDocumentoExt(HttpServletRequest request, Model model){
       ...
       if (mensaje.equals("OK")) {
           return "/segEstRecDocExt/detalleDocExterno";
       } else {
           model.addAttribute("pMensaje", mensaje);
           return "respuesta";
       }       
...
//        String rutaImagen = sc.getRealPath("/reports/logo_onpe.jpg");
      ...
      @RequestMapping(method = RequestMethod.POST, params = "accion=goExportarResumen")
    public @ResponseBody String goExportarResumen(DocExtRecSeguiEstBean docBuscar,HttpServletRequest request,
            BindingResult result, Model model) throws IOException
    {
        ServletContext sc = request.getSession().getServletContext();
        String coReporte = ServletUtility.getInstancia().loadRequestParameter(request, "pformatRepor");
...
//        String rutaImagen = sc.getRealPath("/reports/logo_onpe.jpg");
        ...
        parametros.put("P_USER", usuario.getCoUsuario());
        parametros.put("P_LOGO_DIR", bis.available()==0?null:bis);  
        parametros.put("P_FEC_INICIO", docBuscar.getFeEmiIni());
        parametros.put("P_FEC_FIN", docBuscar.getFeEmiFin());
        
        ReporteBean objReporteBean = seguiEstRecDocExtService.getGenerarReporteReumen(docBuscar, parametros);
        if(bis!=null)bis.close();
        StringBuilder retval = new StringBuilder();
        retval.append("{\"coRespuesta\":\"");        
        retval.append(objReporteBean.getcoRespuesta());

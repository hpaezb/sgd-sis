...
public class CargoEntregaDaoImp extends SimpleJdbcDaoBase implements CargoEntregaDao {
    ...
    public List<CargoEntregaBean> getCargosEntrega(CargoEntregaBean cargo) {
        ...
        sql.append("SELECT TOP 51 A.*");
        sql.append(" FROM ( ");
        sql.append(" SELECT A.NU_ANN_GUI NU_ANN_GUIA, A.NU_GUI NU_GUIA,A.NU_COR_GUI NU_COR_GUIA,CONVERT(VARCHAR(10), A.FE_GUI_MP, 103) FE_GUIA_MP_CORTA,");
        sql.append(" IDOSGD.PK_SGD_DESCRIPCION_DE_LOCAL(A.CO_LOC_DES) DE_LOC_DES,");
        sql.append(" IDOSGD.PK_SGD_DESCRIPCION_DE_DEPENDENCIA(A.CO_DEP_DES) DE_DEP_DES,");
        sql.append(" A.ES_GUI_MP ESTADO_GUIA_MP,");
        sql.append(" IDOSGD.PK_SGD_DESCRIPCION_ESTADOS(A.ES_GUI_MP,'TDTC_GUIA_MP') DE_ESTADO_GUIA_MP");
        sql.append(" FROM IDOSGD.TDTC_GUIA_MP A");
        sql.append(" WHERE");
        ...
        if (cargo.getEsFiltroFecha() != null
                && (cargo.getEsFiltroFecha().equals("1") || cargo.getEsFiltroFecha().equals("3"))) {
            ...
            if (vFechaIni != null && vFechaIni.trim().length() > 0
                ...
                sql.append(" AND A.FE_GUI_MP BETWEEN CONVERT(DATETIME, :pfechaIni, 103) AND CONVERT(DATETIME, :pfechaFin, 103) + 0.99999 ");              
    ...
    public List<DocPedienteEntregaBean> getDocsPendienteEntrega(DocPedienteEntregaBean busqDoc) {
        ...
        sql.append("SELECT TOP 51 A.* FROM");
	sql.append("(");
	sql.append(" SELECT A.NU_ANN,A.NU_EMI,B.NU_DES,A.NU_COR_EMI,IDOSGD.PK_SGD_DESCRIPCION_TI_EMI_EMP(A.NU_ANN, A.NU_EMI) DE_ORI_EMI,");
	sql.append(" CONVERT(VARCHAR(10), A.FE_EMI, 103) FE_EMI_CORTA,IDOSGD.PK_SGD_DESCRIPCION_DE_DOCUMENTO(A.CO_TIP_DOC_ADM) DE_TIP_DOC_ADM,");
        sql.append(" C.NU_DOC,IDOSGD.PK_SGD_DESCRIPCION_DE_DEPENDENCIA(B.CO_DEP_DES) DE_DEP_DES,");
        sql.append(" (");
        sql.append(" CASE ");
...
//            }
    ...
    public String isDocPendienteEnGuiaMp(String pnuAnn,String pnuEmi,String pnuDes){
        ...
        try {
            vReturn = this.jdbcTemplate.queryForObject("select count(1) from IDOSGD.TDTD_GUIA_MP GD WITH (NOLOCK),IDOSGD.TDTC_GUIA_MP G WITH (NOLOCK) \n" +
    ...
    public DocPedienteEntregaBean getDocPendienteEntrega(String pnuAnn,String pnuEmi,String pnuDes){
        ...
        sql.append("SELECT A.NU_ANN,A.NU_EMI,B.NU_DES,C.NU_EXPEDIENTE,IDOSGD.PK_SGD_DESCRIPCION_DE_DOCUMENTO(A.CO_TIP_DOC_ADM) DE_TIP_DOC_ADM,\n" +
                    "C.NU_DOC,CONVERT(VARCHAR(10), A.FE_EMI, 103) FE_EMI_CORTA,IDOSGD.PK_SGD_DESCRIPCION_DE_DEPENDENCIA(B.CO_DEP_DES) DE_DEP_DES,\n" +
                    "IDOSGD.PK_SGD_DESCRIPCION_TI_EMI_EMP(A.NU_ANN, A.NU_EMI) DE_ORI_EMI\n" +
                    "FROM IDOSGD.TDTV_REMITOS A  WITH (NOLOCK) INNER JOIN IDOSGD.TDTV_DESTINOS B WITH (NOLOCK) ON B.NU_ANN=A.NU_ANN AND B.NU_EMI=A.NU_EMI \n" +
                "  IDOSGD.TDTX_REMITOS_RESUMEN C WITH (NOLOCK)  ON C.NU_ANN=B.NU_ANN AND C.NU_EMI=B.NU_EMI "+
    ...
    public String insGuiaMp(GuiaMesaPartesBean guia){
        ...
        sqlUpd.append("INSERT INTO IDOSGD.TDTC_GUIA_MP(\n"
                + "NU_ANN_GUI,NU_GUI,\n"
                ...
                + "FE_USE_CRE)\n"
                + "VALUES(?,?,?,?,?,?,'0',CONVERT(DATETIME,?),?,?,'0',?,GETDATE(),?,GETDATE())");
    ...
    public String updGuiaMp(GuiaMesaPartesBean guia){
        ...
        sqlUpd.append("UPDATE IDOSGD.TDTC_GUIA_MP\n"
                + "SET FE_GUI_MP=CONVERT(DATETIME,?)\n"
                + ",DE_OBS=?\n"
                + ",FE_USE_MOD=GETDATE()\n"
                + ",CO_USE_MOD=?\n"
                + "WHERE\n"
    ...
    public String insDetGuiaMp(DetGuiaMesaPartesBean detGuia){
        String vReturn = "NO_OK";
        StringBuffer sqlUpd = new StringBuffer();
        sqlUpd.append("INSERT INTO IDOSGD.TDTD_GUIA_MP(\n"
                + "NU_ANN_GUI,NU_GUI,\n"
                + "NU_COR,NU_ANN_DES,\n"
                + "NU_EMI,NU_DES,\n"
                + "ES_DET_GUI,ES_ELI,\n"
    ...
    public String getNroGuiaCabecera(String pnuAnnGuia){
        String vReturn="-1";
        try {
            vReturn = this.jdbcTemplate.queryForObject("SELECT ISNULL(MAX(convert(INT, NU_GUI)), 0) + 1\n" +
                                        "FROM IDOSGD.TDTC_GUIA_MP WITH (NOLOCK)  \n" +
                                        "WHERE NU_ANN_GUI=?", 
    ...
    public String getNroCorrelativoGuiaCabecera(String pnuAnnGuia,String pcoDepOri){
        String vReturn="-1";
        try {
            vReturn = this.jdbcTemplate.queryForObject("SELECT ISNULL(MAX(convert(INT, NU_COR_GUI)), 0) + 1\n" +
                                        "FROM IDOSGD.TDTC_GUIA_MP WITH (NOLOCK)  \n" +
                                        "WHERE NU_ANN_GUI=?\n" +
                                        "and CO_DEP_ORI=?", 
    ...
    public String getNroCorrelativoDetGuiaCabecera(String pnuAnnGuia, String pnuGuia){
        String vReturn="-1";
        try {
            vReturn = this.jdbcTemplate.queryForObject("SELECT ISNULL(MAX(convert(INT, NU_COR)), 0) + 1 \n" +
                            "FROM IDOSGD.TDTD_GUIA_MP WITH (NOLOCK)  \n" +
                            "WHERE NU_ANN_GUI=?\n" +
                            "AND NU_GUI=?", 
    ...
    public GuiaMesaPartesBean getGuiaMp(String pnuAnnGuia, String pnuGuia){
        ...
        sql.append("SELECT NU_ANN_GUI NU_ANN,NU_GUI NU_GUIA,CO_DEP_ORI,CO_LOC_ORI,\n" +
                "CO_LOC_DES,IDOSGD.PK_SGD_DESCRIPCION_DE_LOCAL(CO_LOC_DES) DE_LOC_DES,\n" +
                "IDOSGD.PK_SGD_DESCRIPCION_DE_LOCAL(CO_LOC_ORI) DE_LOC_ORI,\n" +
                "IDOSGD.PK_SGD_DESCRIPCION_DE_DEPENDENCIA(CO_DEP_DES) DE_DEP_DES,\n" +
                "IDOSGD.PK_SGD_DESCRIPCION_DE_DEPENDENCIA(CO_DEP_ORI) DE_DEP_ORI,\n" +
                "DE_OBS,CONVERT(VARCHAR(10), FE_GUI_MP, 103) FE_GUI_MP,NU_COR_GUI,\n" +
                "ES_GUI_MP ESTADO_GUIA,IDOSGD.PK_SGD_DESCRIPCION_ESTADOS(ES_GUI_MP,'TDTC_GUIA_MP') DE_ESTADO_GUIA"
                + " FROM IDOSGD.TDTC_GUIA_MP WITH (NOLOCK)  "
    ...
    public List<DocPedienteEntregaBean> getDetalleGuiaMp(String pnuAnnGuia,String pnuGuia){
        ...
        "IDOSGD.PK_SGD_DESCRIPCION_DE_DOCUMENTO(R.CO_TIP_DOC_ADM) DE_TIP_DOC_ADM,\n" +
        "RR.NU_DOC,CONVERT(VARCHAR(10), R.FE_EMI, 103) FE_EMI_CORTA,IDOSGD.PK_SGD_DESCRIPCION_DE_DEPENDENCIA(D.CO_DEP_DES) DE_DEP_DES,\n" +
        "IDOSGD.PK_SGD_DESCRIPCION_TI_EMI_EMP(R.NU_ANN, R.NU_EMI) DE_ORI_EMI,GD.ES_DET_GUI ES_DET_GUIA,\n" +
        "IDOSGD.PK_SGD_DESCRIPCION_FORMAT(D.FE_REC_DOC,'dd/mm/yyyy HH24:MI') FE_REC_DOC_CORTA\n" +
        "FROM IDOSGD.TDTC_GUIA_MP G WITH (NOLOCK)  ,IDOSGD.TDTD_GUIA_MP GD WITH (NOLOCK)  ,IDOSGD.TDTV_REMITOS R WITH (NOLOCK)  ,IDOSGD.TDTV_DESTINOS D WITH (NOLOCK)  ,IDOSGD.TDTX_REMITOS_RESUMEN RR WITH (NOLOCK)  \n" +
        "WHERE\n" +
        "G.NU_ANN_GUI = ?\n" +
        "AND G.NU_GUI = ?\n" +
        "AND GD.NU_ANN_GUI=?\n" +
    ...
    public DocPedienteEntregaBean getDependenciaDestinoDocExtRec(String pnuAnn, String pnuEmi, String pnuDes){
        StringBuffer sql = new StringBuffer();
        DocPedienteEntregaBean doc = null;
        sql.append("SELECT B.CO_LOC_DES,IDOSGD.PK_SGD_DESCRIPCION_DE_LOCAL(CO_LOC_DES) DE_LOC_DES,\n" +
                    "B.CO_DEP_DES,IDOSGD.PK_SGD_DESCRIPCION_DE_DEPENDENCIA(B.CO_DEP_DES) DE_DEP_DES\n" +
                    "FROM IDOSGD.TDTV_REMITOS A  WITH (NOLOCK)  , IDOSGD.TDTV_DESTINOS B WITH (NOLOCK)  \n" +
                    "WHERE A.NU_ANN=? AND A.NU_EMI=? \n" +
    ...
    public List<CargoEntregaBean> getListaReporteBusqueda(CargoEntregaBean cargo) {
        ...
        prutaReporte.append("		[IDOSGD].[PK_SGD_DESCRIPCION_DE_LOCAL](G.CO_LOC_ORI) DE_LOC_ORI, ");
        prutaReporte.append("		G.CO_LOC_DES, ");
        prutaReporte.append("		[IDOSGD].[PK_SGD_DESCRIPCION_DE_LOCAL](G.CO_LOC_DES) DE_LOC_DES, ");
        prutaReporte.append("		G.CO_DEP_ORI, ");
        prutaReporte.append("		[IDOSGD].[PK_SGD_DESCRIPCION_DE_DEPENDENCIA](G.CO_DEP_ORI) DE_DEP_ORI, ");
        prutaReporte.append("		G.CO_LOC_DES, ");
        prutaReporte.append("		[IDOSGD].[PK_SGD_DESCRIPCION_DE_DEPENDENCIA](G.CO_DEP_DES) DE_DEP_DES_CAB, ");
        prutaReporte.append("		GD.NU_COR, ");
        prutaReporte.append("		RR.NU_EXPEDIENTE, ");
        prutaReporte.append("		CASE R.TI_EMI ");
        prutaReporte.append("			WHEN '01' THEN [IDOSGD].[PK_SGD_DESCRIPCION_DE_NOM_EMP](R.CO_EMP_EMI) ");
        prutaReporte.append("			WHEN '02' THEN [IDOSGD].[PK_SGD_DESCRIPCION_DE_PROVEEDOR](R.NU_RUC_EMI) + '-' + R.NU_RUC_EMI ");
        prutaReporte.append("			WHEN '03' THEN [IDOSGD].[PK_SGD_DESCRIPCION_ANI_SIMIL](R.NU_DNI_EMI) + '-' + R.NU_DNI_EMI ");
        prutaReporte.append("			WHEN '04' THEN [IDOSGD].[PK_SGD_DESCRIPCION_OTRO_ORIGEN](R.CO_OTR_ORI_EMI) ");
        ...
        prutaReporte.append("			WHEN '01' THEN [IDOSGD].[PK_SGD_DESCRIPCION_DE_NOM_EMP](d.co_emp_des) ");
        prutaReporte.append("			ELSE NULL ");
        prutaReporte.append("		END de_des, ");
        prutaReporte.append("		CASE d.ti_des ");
        prutaReporte.append("			WHEN '01' THEN [IDOSGD].[PK_SGD_DESCRIPCION_DE_DEPENDENCIA](d.co_dep_des) ");
        prutaReporte.append("			WHEN '02' THEN [IDOSGD].[PK_SGD_DESCRIPCION_DE_PROVEEDOR](d.nu_ruc_des) + ' -RUC:' + d.nu_ruc_des ");
        prutaReporte.append("			WHEN '03' THEN ISNULL([IDOSGD].[PK_SGD_DESCRIPCION_ANI_SIMIL](d.nu_dni_des), ");
        prutaReporte.append("									(SELECT LTRIM(RTRIM(deapp)) + ' ' + LTRIM(RTRIM(deapm))	+ ' ' + LTRIM(RTRIM(denom)) ");
        prutaReporte.append("									 FROM IDOSGD.idtanirs ");
        ...
        prutaReporte.append(" FROM IDOSGD.TDTC_GUIA_MP G WITH (NOLOCK)  , ");
        prutaReporte.append("     IDOSGD.TDTD_GUIA_MP GD WITH (NOLOCK)  , ");
        prutaReporte.append("     IDOSGD.TDTV_DESTINOS D WITH (NOLOCK)  , ");
        prutaReporte.append("     IDOSGD.TDTV_REMITOS R WITH (NOLOCK)  , ");

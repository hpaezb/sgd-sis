...
"                              FROM IDOSGD.RHTM_PER_EMPLEADOS E \n" +
"							  LEFT JOIN IDOSGD.SEG_USUARIOS1 U ON U.CEMP_CODEMP= E.CEMP_CODEMP \n" +
"                             LEFT JOIN IDOSGD.RHTM_PER_EMPLEADOS CRE ON cre.cemp_codemp=e.cemp_id_crea \n" +
"							 LEFT JOIN IDOSGD.RHTM_PER_EMPLEADOS ACT ON ACT.cemp_codemp=e.cemp_id_act "+
    ...
    public int updAdmEmpleado(AdmEmpleadoBean empleado,Usuario usuario) throws Exception{
        ...
        sql.append("UPDATE IDOSGD.RHTM_PER_EMPLEADOS SET "
                ...
                +" CEMP_EST_EMP=?, "
                +" CEMP_INDBAJ=?, "
                +((empleado.getEstado().equals("0")) ? "FEMP_FECBAJ=CURRENT_TIMESTAMP,":"")
                +" CEMP_CO_DEPEND=?, "                
                +" CO_DEPENDENCIA=?, "
                +" CEMP_CO_CARGO=?, "
                +" CEMP_DESCRIP=?,"
                +" CEMP_FE_ACT=CURRENT_TIMESTAMP,CEMP_ID_ACT =?,CEMP_CO_LOCAL=? "
                +" WHERE CEMP_CODEMP=?");
        try {
    ...
    public String nuevoAdmEmpleado(AdmEmpleadoBean empleado, Usuario usuario) throws Exception{
        ...
        try{
            ...
            /*String sql1 = "SELECT lpad(cast (MAX(cast(CEMP_CODEMP as int)+1) as text),5,'0') coEmpleado FROM IDOSGD.RHTM_PER_EMPLEADOS"; 
    ...
    public AdmEmpleadoBean getPersonaDesdeDni(String dni) throws Exception{
        ...
        sql.append(" TO_DATE(A.FENAC,'YYYYMMDD') fechaNacimiento,A.INSEX sexo FROM  IDOSGD.idtanirs A ");
    ...
    public AdmEmpleadoAccesoBean getAcceso(String coEmpleado) throws Exception{
        StringBuffer sql = new StringBuffer();
        sql.append("SELECT COD_USER coUsuario,CDES_USER deUsuario,ES_USUARIO esUsuario,IN_AD inAD FROM IDOSGD.SEG_USUARIOS1 WHERE CEMP_CODEMP='"+coEmpleado+"'");
...
//        }
    ...
    public String saveNuevoAcceso(String coUser, String coEmp, String nuDni,String inAD) throws Exception{
        try{
            System.out.println("INICIANDO REGISTRAR USUARIO==>");
            if(coUser!=null && coUser.length() >0 && coEmp.length()>0){    
                SimpleJdbcCall simpleJdbcCall = 
                new SimpleJdbcCall(this.dataSource)
                        .withSchemaName("IDOSGD")
                        .withFunctionName("PK_SGD_TRAMITE_CREA_USUARIO")            
                ...
                try {
                     ...
                     System.out.println("SE HA EJECUTADO EL  REGISTRO DE USUARIO==>");                     
                     mensaje = (String)out.get("RESULT");
                     System.out.println("RESULTADO==>"+mensaje);
                     System.out.println("PCO_USUARIO==>"+coUser);
                     System.out.println("PCO_EMP==>"+coEmp);
                     System.out.println("PNU_DNI==>"+nuDni);
                     System.out.println("PIN_AD==>"+inAD);
                  } catch (Exception e) {
    ...
    public String updAcceso(String coUser, String coEmp, String nuDni,String inAD)  {
        ...
        sqlUpd.append("update IDOSGD.SEG_USUARIOS1 \n"
                + "set IN_AD=?\n"
                + "where\n"
                + " CEMP_CODEMP=? \n");
        try {
        ...
        try{
            if(acceso.getCoEmpleado()!=null && acceso.getCoEmpleado().length()>0){
                ...
                if(acceso_tmp!=null){
                    ...
                    SimpleJdbcCall simpleJdbcCall = new SimpleJdbcCall(this.dataSource).withFunctionName("IDOSGD.PK_SEG_SEGURIDAD_BORRAR_USUARIO")        
                        .withoutProcedureColumnMetaDataAccess()
    ...
    public boolean restablecerAcceso(AdmEmpleadoAccesoBean acceso, AdmEmpleadoBean empleado) throws Exception{
        try{
            if(acceso.getCoEmpleado()!=null && acceso.getCoEmpleado().length()>0){
                ...
                if(acceso_tmp!=null){
                    //PK_SEG_SEGURIDAD.borrar_usuario(pco_usuario varchar2);
                    SimpleJdbcCall simpleJdbcCall = new SimpleJdbcCall(this.dataSource).withFunctionName("IDOSGD.PK_SEG_SEGURIDAD_RESET_PASSWORD")
                        .withoutProcedureColumnMetaDataAccess()
    ...
    public String getNextCoEmpleado(){
        String vReturn = "1";
        //Obtiene codigo de empleado
        String sql = "SELECT lpad(cast (MAX(cast(CEMP_CODEMP as int)+1) as text),5,'0') coEmpleado FROM IDOSGD.RHTM_PER_EMPLEADOS";        
        try {

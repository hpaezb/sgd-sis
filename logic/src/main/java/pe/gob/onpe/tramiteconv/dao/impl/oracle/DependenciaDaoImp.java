...
public class DependenciaDaoImp extends SimpleJdbcDaoBase implements DependenciaDao{
    ...
    public DependenciaBean getDependencia(String coDep) {
        ...
        StringBuffer sql = new StringBuffer();
        sql.append("SELECT DE.co_dependencia,DE.de_dependencia,DE.in_baja,DE.co_nivel,\n" +
                "de_corta_depen,DE.co_empleado,PK_SGD_DESCRIPCION.DE_NOM_EMP(DE.co_empleado) de_empleado,\n" +
                ...
                "de_cargo_completo,DE.co_depen_padre,PK_SGD_DESCRIPCION.DE_DEPENDENCIA(co_depen_padre) de_depen_padre,\n" +
                "DE.titulo_dep,\n" +
                "DE.ti_dependencia tiDependencia, \n"+
                " de.id_crea ,"
                ...
                " FROM RHTM_DEPENDENCIA DE \n" +
                " LEFT JOIN RHTM_PER_EMPLEADOS CRE ON cre.cemp_codemp=de.id_crea LEFT JOIN RHTM_PER_EMPLEADOS ACT ON ACT.cemp_codemp=de.id_act "+
    ...
    public String insDependencia(DependenciaBean dep){
        if(dep.getTiDependencia().equals("1")){ 
            String sql1 = "(SELECT lpad(to_char(to_number(COALESCE(MAX(CO_DEPENDENCIA),'0'))+1),5,'0') FROM RHTM_DEPENDENCIA)";

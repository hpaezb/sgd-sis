...
public class RequisitoDaoImpl extends SimpleJdbcDaoBase implements RequisitoDao {
    ...
    public List<RequisitoBean> getRequisitoList() {
        ...
        sql.append("        TO_CHAR(FE_CREA_AUDI, 'dd/mm/yyyy') FE_CREA_AUDI, ");
        sql.append("        US_MODI_AUDI, ");
        sql.append("        TO_CHAR(FE_MODI_AUDI, 'dd/mm/yyyy') FE_MODI_AUDI ");
    ...
    public String insRequisito(RequisitoBean requisito, String codusuario) {
        ...
        try {
            ...
            String seq = "SELECT lpad(cast((cast(coalesce(MAX(COD_REQ), '0') as int) + 1) as text), 4, '0') FROM IDOSGD.TDTR_REQUISITOS ";
    ...
    public String insTupaRequisito(RequisitoBean requisito, String codusuario) {
        ...
        sql.append("values (?, ?, CAST(? AS NUMERIC), ?, ?, ?, CURRENT_TIMESTAMP, CURRENT_TIMESTAMP, ?) ");
        try {
            this.jdbcTemplate.update(sql.toString(), new Object[] {

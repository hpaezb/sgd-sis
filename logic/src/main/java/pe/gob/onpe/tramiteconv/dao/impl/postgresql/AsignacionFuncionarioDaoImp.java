...
public class AsignacionFuncionarioDaoImp extends SimpleJdbcDaoBase implements AsignacionFuncionarioDao {
    ...
    public String[] insAsignacionFuncionario(AsignacionFuncionarioBean asignacionFuncionarioBean) {
        ...
        //Get sequence
        sqlSeq.append("SELECT NEXTVAL('IDOSGD.TDTV_ASIGNACION_FUNCIONARIO_CO') ");
        sql.append("INSERT\n"
    ...
    public List<AsignacionFuncionarioBean> getListAsignacionFuncionario(AsignacionFuncionarioBean asignacionFuncionarioBean) {
        ...
        query.append("SELECT ASF.CO_ASIGNACION_FUNCIONARIO, \n"
                + "	ASF.CO_DEPENDENCIA, \n"
                + "	(SELECT DE_DEPENDENCIA FROM IDOSGD.RHTM_DEPENDENCIA WHERE CO_DEPENDENCIA=ASF.CO_DEPENDENCIA) AS DE_DEPENDENCIA, \n"
                + "	ASF.DE_DOC_ASIGNA, \n"
                + "	ASF.CO_EMPLEADO, \n"
                + "	(SELECT TRIM(COALESCE(CEMP_APEPAT,''))||' '||TRIM(COALESCE(CEMP_APEMAT,''))||' '||TRIM(COALESCE(CEMP_DENOM,'')) FROM IDOSGD.RHTM_PER_EMPLEADOS WHERE CEMP_CODEMP=ASF.CO_EMPLEADO)DE_EMPLEADO,  \n"
                + "	ASF.FE_INICIO, \n"
                + "	ASF.FE_FIN, \n"
                + "	ASF.CO_TIPO_ENCARGO, \n"
                +"      (SELECT CELE_DESELE FROM IDOSGD.SI_ELEMENTO WHERE CTAB_CODTAB='CO_TIPO_ENC' AND CELE_CODELE=ASF.CO_TIPO_ENCARGO) DE_TIPO_ENCARGO,"
                + "	ASF.CO_USE_CRE, \n"
                + "	ASF.FE_USE_CRE, \n"
                + "	ASF.CO_USE_MOD, \n"
                + "	ASF.FE_USE_MOD, \n"
                + "	ASF.CO_ESTADO, \n"
                + "	ASF.IN_ASIGNACION,\n"
                + "	(SELECT DE_EST FROM IDOSGD.TDTR_ESTADOS WHERE DE_TAB='TDTV_ASIGNACION_FUNCIONARIO' AND CO_EST=ASF.IN_ASIGNACION) DE_ASIGNACION\n"
        ...
        if(asignacionFuncionarioBean.getFeRegistraInicio()!=null &&asignacionFuncionarioBean.getFeRegistraFin()!=null){
            query.append("     AND ASF.FE_USE_CRE BETWEEN TO_DATE(:pfeInicio,'dd/mm/yyyy') AND TO_DATE(:pfeFin,'dd/mm/yyyy') + TIME '23:59:59'");
        }       
    ...
    public AsignacionFuncionarioBean getFuncionarioPorDependencia(String coDependencia) {
        ...
        String query = "SELECT DEP.CO_DEPENDENCIA,\n"
                +" DEP.CO_EMPLEADO,\n"
                + "(SELECT EMP.CEMP_APEPAT||' '||EMP.CEMP_APEMAT||' '||EMP.CEMP_DENOM  FROM IDOSGD.RHTM_PER_EMPLEADOS EMP WHERE EMP.CEMP_CODEMP=DEP.CO_EMPLEADO)AS DE_EMPLEADO,\n"
                + "DEP.CO_TIPO_ENCARGATURA,\n"
                + "(SELECT CELE_DESELE FROM IDOSGD.SI_ELEMENTO WHERE SI_ELEMENTO.CELE_CODELE=DEP.CO_TIPO_ENCARGATURA AND SI_ELEMENTO.CTAB_CODTAB='CO_TIPO_ENC') DE_TIPO_ENCARGO \n"
                + "FROM IDOSGD.RHTM_DEPENDENCIA DEP\n"

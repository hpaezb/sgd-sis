...
public class AdmEmpleadoDaoImp extends SimpleJdbcDaoBase implements AdmEmpleadoDao{
    ...
    /*private String sql_base = "SELECT E.CEMP_CODEMP co_Empleado, E.CEMP_APEPAT ap_Paterno, E.CEMP_APEMAT ap_Materno,"
        ...
        private String sql_base = " SELECT E.CEMP_CODEMP co_Empleado, E.CEMP_APEPAT ap_Paterno, E.CEMP_APEMAT ap_Materno,\n" +
"                                            E.CEMP_DENOM nombres, E.CEMP_NU_DNI dni,E.CEMP_TIPSEX sexo, convert(nvarchar,E.FEMP_FECNAC,103)  fecha_Nacimiento, \n" +
"                                            E.CEMP_EMAIL email,E.CEMP_EST_EMP estado, \n" +
"                                            E.CEMP_CO_DEPEND co_Dependencia, E.CEMP_CO_CARGO coCargo, \n" +
"                                            IDOSGD.PK_SGD_DESCRIPCION_de_dependencia(E.CEMP_CO_DEPEND) de_Dependencia, \n" +
"                                            IDOSGD.PK_SGD_DESCRIPCION_de_cargo(E.CEMP_CO_CARGO) de_Cargo ,\n" +
...
"                            ,CONVERT(varchar(20), e.cemp_fe_crea, 120)    crea_Fecha \n" +
"                            ,e.cemp_id_act \n" +
"                            , ACT.CEMP_DENOM +' '+ ACT.CEMP_APEPAT act_Usuario \n" +
"                            ,CONVERT(varchar(20), e.cemp_fe_act, 120)  act_Fecha , LO.de_nombre_local NO_LOCAL, E.CEMP_CO_LOCAL CO_LOCAL \n" +
"                              FROM IDOSGD.RHTM_PER_EMPLEADOS E LEFT JOIN IDOSGD.SEG_USUARIOS1 U ON U.CEMP_CODEMP= E.CEMP_CODEMP \n" +
"                             LEFT JOIN IDOSGD.RHTM_PER_EMPLEADOS CRE ON cre.cemp_codemp=e.cemp_id_crea LEFT JOIN IDOSGD.RHTM_PER_EMPLEADOS ACT ON ACT.cemp_codemp=e.cemp_id_act "+
    ...
    public int updAdmEmpleado(AdmEmpleadoBean empleado,Usuario usuario) throws Exception{
        StringBuffer sql = new StringBuffer();
        sql.append("SET LANGUAGE Spanish; ");
        sql.append("UPDATE IDOSGD.RHTM_PER_EMPLEADOS SET "
                +" CEMP_NU_DNI=?,"
                +" CEMP_APEPAT=?,"
                +" CEMP_APEMAT=?,"
                +" CEMP_DENOM=?,"
                +" FEMP_FECNAC=cast(? as datetime2),"
                +" CEMP_TIPSEX=?, "
                +" CEMP_EMAIL=?, "
                +" CEMP_EST_EMP=?, "
                +" CEMP_INDBAJ=?, "
                ...
                +" CEMP_CO_CARGO=?, "
                +" CEMP_DESCRIP=?,"
                +" CEMP_FE_ACT=getdate(),CEMP_ID_ACT =?,CEMP_CO_LOCAL=? "
                +" WHERE CEMP_CODEMP=?");
        try {
            String fechaNaci=empleado.getFechaNacimiento2();
            int upd = this.jdbcTemplate.update(sql.toString(),new Object[]{empleado.getDni(), empleado.getApPaterno().toUpperCase(),empleado.getApMaterno().toUpperCase(),
                ...
                empleado.getNombres().toUpperCase(),(fechaNaci.equals("")?null:fechaNaci),empleado.getSexo(),empleado.getEmail().toLowerCase(),empleado.getEstado(),empleado.getEstado(),

...
public class MensajeriaDaoImp extends SimpleJdbcDaoBase  implements MensajeriaDao{
    ...
    public List<DocumentoRecepMensajeriaBean> getDocumentoRecepMensajeria(BuscarDocumentoRecepMensajeriaBean buscarDocumentoRecepMensajeriaBean) {
        ...
        sql.append("SELECT  TOP 100 ROW_NUMBER() OVER(ORDER BY  NU_ANN,NU_EMI DESC )   fila, NU_ANN,NU_EMI,\n" +
                "     IDOSGD.PK_SGD_DESCRIPCION_DE_DOCUMENTO(X.CO_TIP_DOC_ADM) DE_TIP_DOC_ADM,FE_EMI_CORTA,TOTAL_DESTINO,TOTAL_ENVIADO,TOTAL_PENDIENTE,\n" +
                "        IDOSGD.PK_SGD_DESCRIPCION_DE_DEPENDENCIA (X.CO_DEP_EMI) DE_DEPENDENCIA,\n" +                
                "            DE_ASU,\n" +
                "       CASE  X.TI_EMI WHEN '01' THEN  X.NU_DOC_EMI + '-' + X.NU_ANN + '/' + X.DE_DOC_SIG    \n" +
                "            WHEN '05' THEN X.NU_DOC_EMI + '-' + X.NU_ANN + '/' + X.DE_DOC_SIG ELSE X.DE_DOC_SIG END NU_DOC,  "+
                "       CASE  X.NU_CANDES WHEN 1 THEN (SELECT RETORNO  FROM IDOSGD.PK_SGD_DESCRIPCION_TI_DES_EMP (X.NU_ANN, X.NU_EMI))  \n" +
                "         	ELSE (SELECT RETORNO  FROM IDOSGD.PK_SGD_DESCRIPCION_TI_DES_EMP_V (X.NU_ANN, X.NU_EMI) ) END 	DE_EMP_DES,  \n" +
                 
                "         DE_ES_DOC_EMI_MP, DOC_ESTADO_MSJ, FEC_RECEPMP,      \n" +  //IDOSGD.PK_SGD_DESCRIPCION_ESTADOS_MP(X.ES_DOC_EMI,'TDTV_REMITOS')
                "         IDOSGD.PK_SGD_DESCRIPCION_DE_LOCAL(X.CO_LOC_EMI) DE_LOC_EMI \n" +
                "        ,FEC_ENVIOMSJ, DE_AMBITO,DE_TIP_ENV,NU_SER_MSJ, FE_PLA_MSJ,DIAS_PLA_DEVO  diasDevoluvion ,DIAS_PLA_ENTR diasEntrega,CONVERT(varchar(20), FE_ENV_MES , 120)   fechaenvioamensajeria \n"+
                "         FROM ( \n" +
                "         SELECT A.NU_ANN,A.NU_EMI ,CONVERT(varchar(20), A.FE_EMI, 103)   FE_EMI_CORTA,\n" +
                "         A.DE_ASU,A.CO_DEP_EMI,A.CO_EMP_EMI,A.CO_OTR_ORI_EMI,A.CO_TIP_DOC_ADM,A.NU_DOC_EMI,A.DE_DOC_SIG,\n" +
                "          DE_ES_DOC_EMI_MP, DOC_ESTADO_MSJ,CONVERT(varchar(20), FEC_RECEPMP , 120)  FEC_RECEPMP, \n" +
                "         A.TI_EMI,A.NU_RUC_EMI,A.NU_DNI_EMI,A.NU_CANDES,A.ES_DOC_EMI,A.CO_LOC_EMI,\n" +
                "         A.CO_DEP,CAST(ISNULL(TOTAL_DESTINO,0) AS varchar(20)) TOTAL_DESTINO ,CAST(ISNULL(TOTAL_ENVIADO,0) AS varchar(20))TOTAL_ENVIADO ,CAST(ISNULL(TOTAL_PENDIENTE,0) AS varchar(20)) TOTAL_PENDIENTE\n" +
                "        ,FEC_ENVIOMSJ, DE_AMBITO,DE_TIP_ENV,NU_SER_MSJ, FE_PLA_MSJ \n"+
                "        ,  C.DIAS_PLA_DEVO,C.DIAS_PLA_ENTR,A.FE_ENV_MES \n"+
                "          FROM IDOSGD.TDTV_REMITOS A  WITH (NOLOCK) INNER JOIN IDOSGD.TDTX_REMITOS_RESUMEN B WITH (NOLOCK)  ON A.NU_ANN = B.NU_ANN AND A.NU_EMI = B.NU_EMI\n" +
                "        LEFT JOIN (SELECT NU_ANN,NU_EMI, ISNULL(COUNT(NU_DES),0) TOTAL_DESTINO FROM IDOSGD.TDTV_DESTINOS WITH (NOLOCK)  WHERE TI_DES<>'01' AND ES_DOC_REC IN  ('0','1','2','3','4') GROUP BY NU_ANN,NU_EMI ) E ON A.NU_ANN = E.NU_ANN AND A.NU_EMI = E.NU_EMI\n" +
                "        LEFT JOIN (SELECT NU_ANN,NU_EMI,ISNULL(COUNT(NU_DES),0) TOTAL_ENVIADO ,M.FEC_ENVIOMSJ,M.DIAS_PLA_DEVO,M.DIAS_PLA_ENTR, M.DE_AMBITO,M.DE_TIP_ENV,M.NU_SER_MSJ+'-'+M.AN_SER_MSJ NU_SER_MSJ, M.FE_PLA_MSJ\n" +
                "                   FROM IDOSGD.TDTV_DESTINOS D  WITH (NOLOCK) LEFT JOIN IDOSGD.TD_MENSAJERIA M  WITH (NOLOCK) ON M.NU_MSJ=D.NU_MSJ\n" +
                ...
                "        LEFT JOIN (SELECT NU_ANN,NU_EMI, ISNULL(COUNT(NU_DES),0) TOTAL_PENDIENTE FROM IDOSGD.TDTV_DESTINOS WITH (NOLOCK)  WHERE TI_DES <>'01' AND ES_DOC_REC IN ('0','1') GROUP BY NU_ANN,NU_EMI) D ON A.NU_ANN = D.NU_ANN AND A.NU_EMI = D.NU_EMI\n" +
                "     LEFT JOIN (SELECT CO_EST,DE_EST DE_ES_DOC_EMI_MP FROM IDOSGD.TDTR_ESTADOS WITH (NOLOCK)  WHERE DE_TAB='ENVIO_DOCUMENTO_MSJ') EST ON EST.CO_EST= A.DOC_ESTADO_MSJ \n"+
...
//        sql.append(" AND A.ES_DOC_EMI<>9");
        ...
        if (pTipoBusqueda.equals("0") || bBusqFiltro) {
            ...
            if (buscarDocumentoRecepMensajeriaBean.getEsFiltroFechaEnvMsj()!= null  && (pEsFiltroFechaENVmSJ.equals("1") || pEsFiltroFechaENVmSJ.equals("3"))) {
                ...
                if (vFeEmiIni != null && vFeEmiIni.trim().length() > 0  && vFeEmiFin != null && vFeEmiFin.trim().length() > 0) {            
                    sql.append(" AND A.FE_EMI BETWEEN CONVERT(DATETIME, :pFeEmiIniEnv, 103) AND CONVERT(DATETIME, :pFeEmiFinEnv, 103) + 0.99999 ");
        ...
        //if (pTipoBusqueda.equals("1")) {
            ...
            if (buscarDocumentoRecepMensajeriaBean.getBusDesti()!= null && buscarDocumentoRecepMensajeriaBean.getBusDesti().trim().length() > 0  ) {
                sql.append("  AND UPPER ( (CASE A.NU_CANDES WHEN 1  THEN (SELECT RETORNO FROM IDOSGD.PK_SGD_DESCRIPCION_TI_DES_EMP (A.NU_ANN, A.NU_EMI) )  ELSE  ( SELECT RETORNO FROM IDOSGD.PK_SGD_DESCRIPCION_TI_DES_EMP_V (A.NU_ANN, A.NU_EMI) )  END  ) \n" +                        
                        "   )  LIKE '%'+UPPER(:pDesti)+'%' ");
    ...
    public List<DestiDocumentoEnvMensajeriaBean> getLstDestinoEnvMensajeria(String pnuAnnpnuEmi) {
        ...
        sql.append("select ROW_NUMBER() OVER(ORDER BY  a.nu_ann,a.nu_emi )  fila, a.nu_ann,a.nu_emi,a.nu_des,a.co_loc_des co_local, substring(IDOSGD.PK_SGD_DESCRIPCION_DE_LOCAL(a.co_loc_des), 1, 100)  de_local, \n" +
"                    B.co_dep_emi co_dependencia,\n" +
"                     substring(IDOSGD.PK_SGD_DESCRIPCION_DE_DEPENDENCIA(B.co_dep_emi), 1, 100)  de_dependencia, \n" +
"                  --  a.co_emp_des co_empleado,\n" +
"                    --NVL2(a.co_emp_des,substring(PK_SGD_DESCRIPCION.DE_NOM_EMP(a.co_emp_des), 1, 100),NULL) de_empleado, \n" +
"                    IDOSGD.PK_SGD_DESCRIPCION_DE_DOCUMENTO(B.CO_TIP_DOC_ADM) documento,\n" +
...
" CASE A.TI_DES WHEN '01' THEN LTRIM(DEP.NODEP)+'/'+LTRIM(DEP.NOPRV)+'/'+LTRIM(DEP.NODIS) "+                
" WHEN '02' THEN LTRIM(UBIDESTINO.NODEP)+'/'+LTRIM(UBIDESTINO.NOPRV)+'/'+LTRIM(UBIDESTINO.NODIS) "+                
" WHEN '03' THEN LTRIM(UBIDESTINO.NODEP)+'/'+LTRIM(UBIDESTINO.NOPRV)+'/'+LTRIM(UBIDESTINO.NODIS) "+                                
" WHEN '04' THEN LTRIM(UBIDESTINO.NODEP)+'/'+LTRIM(UBIDESTINO.NOPRV)+'/'+LTRIM(UBIDESTINO.NODIS) "+                                
" WHEN '05' THEN LTRIM(DEP.NODEP)+'/'+LTRIM(DEP.NOPRV)+'/'+LTRIM(DEP.NODIS) END departamento,CCOD_TIPO_UBI AS ambito, "+                                                 
"  CASE  B.TI_EMI WHEN '01' THEN    B.NU_DOC_EMI + '-' + B.NU_ANN + '/' + B.DE_DOC_SIG  \n" +
"    WHEN '05' THEN   B.NU_DOC_EMI + '-' + B.NU_ANN + '/' + B.DE_DOC_SIG   \n" +
"    ELSE  B.DE_DOC_SIG END  NU_DOC,  \n" +
"         \n" +                 
"          CONVERT(varchar(20), B.FE_EMI, 103)   FE_EMI_CORTA,a.CCOD_DPTO,a.CCOD_PROV,a.CCOD_DIST \n" +
"                    FROM IDOSGD.tdtv_destinos a  WITH (NOLOCK) INNER JOIN IDOSGD.TDTV_REMITOS B WITH (NOLOCK)  ON a.NU_ANN = B.NU_ANN AND a.NU_EMI = B.NU_EMI\n" +
...
"                                FROM IDOSGD.RHTM_DEPENDENCIA D WITH (NOLOCK)  \n" +
"                                INNER JOIN IDOSGD.SITM_LOCAL_DEPENDENCIA DL WITH (NOLOCK)  ON DL.CO_DEP=D.CO_DEPENDENCIA\n" +
"                                INNER JOIN IDOSGD.SI_MAE_LOCAL L WITH (NOLOCK)  ON L.CCOD_LOCAL=DL.CO_LOC\n" +
"                                LEFT JOIN IDOSGD.IDTUBIAS U  WITH (NOLOCK) ON U.UBDEP+U.UBPRV+U.UBDIS =L.CO_UBIGEO  ) DEP ON DEP.CO_DEPENDENCIA=A.co_dep_DES AND DEP.CCOD_LOCAL=a.co_loc_des " +
                " LEFT JOIN IDOSGD.IDTUBIAS UBIDESTINO  WITH (NOLOCK) ON UBIDESTINO.UBDEP+UBIDESTINO.UBPRV+UBIDESTINO.UBDIS =a.CCOD_DPTO+a.CCOD_PROV+a.CCOD_DIST  \n"+
    ...
    public List<TipoElementoMensajeriaBean> getlistTipoElementoMensajeria(String tipo){
                StringBuffer sql = new StringBuffer();        
        sql.append("select CELE_DESELE de_destinatario,CELE_CODELE co_destinatario from IDOSGD.SI_ELEMENTO WHERE CTAB_CODTAB=? \n" +
    ...
    public List<TipoElementoMensajeriaBean> getListResponsableMensajeria(String tipo,String Ambito,String tipoEnvio){
        ...
        sql.append("SELECT CPRO_RUC codigo ,CPRO_RAZSOC nombre FROM IDOSGD.LG_PRO_PROVEEDOR WHERE CPRO_RUC IN (  select CU.CELE_DESELE from IDOSGD.SI_ELEMENTO CU \n" +
                    ...
                    " INNER JOIN IDOSGD.SI_ELEMENTO TENV ON CU.cele_codele3=TENV.cele_codele AND TENV.CTAB_CODTAB='RE_ENV_MSJ_MENS' \n" +
    ...
    public String insMensajeriaDocumento(DocumentoRecepMensajeriaBean documentoMensajeria) {
        ...
        sqlQry2.append(" SELECT CONVERT(VARCHAR(20),GETDATE(),103) ; ");
        sqlIns.append(" SET LANGUAGE Español; ");
        sqlIns.append(" DECLARE @CODIGO NUMERIC;SELECT @CODIGO=ISNULL(MAX(NU_MSJ),0)+1 FROM IDOSGD.td_mensajeria WITH (NOLOCK) ; ");
        sqlIns.append("INSERT INTO  IDOSGD.td_mensajeria ( nu_msj,fe_reg_msj,co_use_cre,de_ambito,de_tip_msj,re_env_msj,\n" +
                    ...
                    "    VALUES(@CODIGO,GETDATE(),?,?,?,?,?,?,?,CONVERT(DATETIME,?),CONVERT(DATETIME,?) ");
        sqlIns.append(",CONVERT(DATETIME,?),CONVERT(DATETIME,?),'0',?,?,?,?,?); ");
        sqlIns.append(" UPDATE IDOSGD.TDTV_DESTINOS SET ES_DOC_REC='2', NU_MSJ=@CODIGO, FEC_ENVIOMSJ= GETDATE()  WHERE  NU_ANN+NU_EMI+CAST(NU_DES AS NVARCHAR(20)) in ( "+ documentoMensajeria.getCodigo()+" ); ");
        try{                   
            String snufecha = this.jdbcTemplate.queryForObject(sqlQry2.toString(), String.class);                
            documentoMensajeria.setFeregmsj(snufecha);
            
            sqlIns.append(" UPDATE IDOSGD.TDTV_REMITOS  SET  FEC_ENVIODEP=GETDATE(),DOC_ESTADO_MSJ='2' \n" +
                    " WHERE  NU_ANN+NU_EMI IN ( SELECT NU_ANN+NU_EMI FROM  IDOSGD.TDTV_DESTINOS  WITH (NOLOCK) WHERE NU_ANN+NU_EMI+CAST(NU_DES AS NVARCHAR(20)) in (  "+documentoMensajeria.getCodigo()+"  ) ) ; ");             
            sqlIns.append(" UPDATE IDOSGD.TDTV_REMITOS  SET  FEC_ENVIODEP=GETDATE(),DOC_ESTADO_MSJ='3' \n" +
"                     WHERE  NU_ANN+NU_EMI IN (SELECT NU_ANN+NU_EMI FROM  IDOSGD.TDTV_DESTINOS  WITH (NOLOCK) WHERE ES_DOC_REC<>'2' \n" +
"                     and NU_ANN+NU_EMI in ( SELECT NU_ANN+NU_EMI FROM  IDOSGD.TDTV_DESTINOS  WITH (NOLOCK) WHERE   NU_ANN+NU_EMI+CAST(NU_DES AS NVARCHAR(20)) in (  "+documentoMensajeria.getCodigo()+"  )  ) ) ; ");
            this.jdbcTemplate.update(sqlIns.toString(), new Object[]{//snuNumsj,snufecha,
             ...
             String snuNumsj = this.jdbcTemplate.queryForObject(" SELECT CAST( MAX(NU_MSJ) AS NVARCHAR(20)) FROM IDOSGD.TDTV_DESTINOS WITH (NOLOCK)  WHERE  NU_ANN+NU_EMI+CAST(NU_DES AS NVARCHAR(20)) in ( "+ documentoMensajeria.getCodigo()+" ) ", String.class); 
...
//                documentoMensajeria.getHoenvmsj(),documentoMensajeria.getFeplamsj(),documentoMensajeria.getHoplamsj(),documentoMensajeria.getDiasEntrega(),documentoMensajeria.getDiasDevoluvion()});
    ...
    public String updMensajeriaDocumentoRecibir(String codigo) {
        ...
        sqlIns.append("UPDATE IDOSGD.TDTV_REMITOS  SET FEC_RECEPMP=GETDATE(),DOC_ESTADO_MSJ='1'  WHERE  NU_ANN+NU_EMI IN ("+codigo+" ) ");
        sqlUpd.append("UPDATE IDOSGD.TDTV_DESTINOS SET ES_DOC_REC='1'  WHERE TI_DES <>'01' AND NU_ANN+NU_EMI in ( "+ codigo +" ) ");
        try{
    ...
    public String updMensajeriaDocumentoDevolver(String codigo) {
        ...
        StringBuffer sqlIns = new StringBuffer();
        StringBuffer sqlUpd = new StringBuffer();  
        sqlIns.append("UPDATE IDOSGD.TDTV_REMITOS  SET  DOC_ESTADO_MSJ=NULL,COD_DEP_MSJ=NULL ,TI_ENV_MSJ=NULL  WHERE  NU_ANN+NU_EMI IN ("+codigo+" ) ");        
        sqlUpd.append("UPDATE IDOSGD.TDTV_DESTINOS SET ES_DOC_REC='0'  WHERE TI_DES <>'01' AND NU_ANN+NU_EMI in ( "+ codigo +" ) ");
        try{
            this.jdbcTemplate.update(sqlIns.toString() );
            this.jdbcTemplate.update(sqlUpd.toString() ); 
            vReturn = "OK";
    ...
    public String selectCalcularFechaPlazo(DocumentoRecepMensajeriaBean documentoMensajeria) {
        ...
        StringBuffer sqlQry2 = new StringBuffer();
        sqlQry0.append("SELECT CAST(NELE_NUMSEC AS VARCHAR(20)) FROM IDOSGD.SI_ELEMENTO WHERE CTAB_CODTAB='DE_TIP_MSJ_MENS' AND CELE_DESELE='"+documentoMensajeria.getDetipmsj()+"'");  
        if(documentoMensajeria.getDetipmsj().equals("COURRIER")){
           sqlQry1.append("SELECT  CELE_DESCOR FROM  IDOSGD.si_elemento WHERE CTAB_CODTAB='DE_COURRIER' AND cele_desele='"+documentoMensajeria.getDeNuRuc()+"'  \n" +
                    "AND cele_codele2 IN (SELECT  cele_codele FROM  IDOSGD.si_elemento WHERE CTAB_CODTAB='DE_AMBITO_MENS' AND cele_desele='"+documentoMensajeria.getDeambito()+"') \n" +
                    "AND cele_codele3 IN (SELECT  cele_codele FROM  IDOSGD.si_elemento WHERE CTAB_CODTAB='RE_ENV_MSJ_MENS' AND cele_desele='"+documentoMensajeria.getDetipenv()+"') "  );
        }
        else {
           sqlQry1.append("SELECT  CELE_DESCOR FROM  IDOSGD.si_elemento WHERE CTAB_CODTAB='"+documentoMensajeria.getDetipmsj()+"' AND cele_desele='"+documentoMensajeria.getDeNuRuc()+"'  ");
        }
        try{
            ...
            if(confCalculaPena.equals("1")){
                ...
                sqlQry2.append("select CONVERT(varchar(20),IDOSGD.PK_SGD_CONSULTAS_CALCULAR_DIAS_HABILES(CAST('"+documentoMensajeria.getFecenviomsj()+"' AS DATE),"+sdias+"),103)  ");  
                String snufecha = this.jdbcTemplate.queryForObject(sqlQry2.toString(), String.class); 
...
"                    IDOSGD.PK_SGD_DESCRIPCION_ESTADOS (D.ES_DOC_REC,'TD_MENSAJERIA') docEstadoMsj,FE_PLA_MSJ,\n" +
"                     CAST(B.NU_COR_EMI AS NVARCHAR(10))+'-'+CAST(D.NU_DES AS NVARCHAR(10))+'-1' as nu_Acta_Vis1,CAST(B.NU_COR_EMI AS NVARCHAR(10))+'-'+CAST(D.NU_DES AS NVARCHAR(10))+'-2' as nu_Acta_Vis2, \n" +
"                      ISNULL(CONVERT(NVARCHAR,DM1.FEC_NOT_ACTA,103),' ') as fe_Acta_Vis1,ISNULL(DM1.ES_NOT_ACTA,2) as es_Acta1_msj, \n" +
"                      ISNULL(CONVERT(NVARCHAR,DM2.FEC_NOT_ACTA,103),' ') as fe_Acta_Vis2,ISNULL(DM2.ES_NOT_ACTA,2) as es_Acta2_msj, \n" +
"                     ISNULL(DM1.DE_RUT_ORI,' ') as archivo_Acta1, \n" +
"                     ISNULL(DM2.DE_RUT_ORI,' ') as archivo_Acta2, \n" +
"                     ISNULL(CAST(CARGO.NU_ANE AS NVARCHAR(10)),' ') as archivo_Cargo  \n" +
"                     FROM IDOSGD.TDTV_DESTINOS D  WITH (NOLOCK) \n" +
"                     INNER JOIN IDOSGD.TDTV_REMITOS B  WITH (NOLOCK) ON D.NU_ANN = B.NU_ANN AND D.NU_EMI = B.NU_EMI \n" +
"                     INNER JOIN IDOSGD.TD_MENSAJERIA M  WITH (NOLOCK) ON M.NU_MSJ=D.NU_MSJ\n" +
"                     LEFT JOIN IDOSGD.RHTM_PER_EMPLEADOS R WITH (NOLOCK)  ON R.CEMP_NU_DNI=M.RE_ENV_MSJ \n" +
"                     LEFT JOIN IDOSGD.LG_PRO_PROVEEDOR P  WITH (NOLOCK) ON P.CPRO_RUC=M.RE_ENV_MSJ  \n" +
"                     LEFT JOIN IDOSGD.TD_DET_MENSAJERIA DM1 WITH (NOLOCK)  ON DM1.NU_ANN=D.NU_ANN AND DM1.NU_EMI=D.NU_EMI AND DM1.NU_DES=D.NU_DES AND DM1.NU_ACTA=CAST(B.NU_COR_EMI AS NVARCHAR(10))+'-'+CAST(D.NU_DES AS NVARCHAR(10))+'-1' \n" +
"                     LEFT JOIN IDOSGD.TD_DET_MENSAJERIA DM2 WITH (NOLOCK)  ON DM2.NU_ANN=D.NU_ANN AND DM2.NU_EMI=D.NU_EMI AND DM2.NU_DES=D.NU_DES AND DM2.NU_ACTA=CAST(B.NU_COR_EMI AS NVARCHAR(10))+'-'+CAST(D.NU_DES AS NVARCHAR(10))+'-2' \n" +
"                     LEFT JOIN IDOSGD.TDTV_ANEXOS CARGO  WITH (NOLOCK) ON CARGO.NU_ANN=D.NU_ANN AND CARGO.NU_EMI=D.NU_EMI AND CARGO.NU_DES=D.NU_DES AND CARGO.DE_DET='CARGO_NOTIFICACION'+CAST(D.NU_DES AS NVARCHAR(10))+'.pdf' "+                
    ...
    public List<DestiDocumentoEnvMensajeriaBean> getEditLstDetalleMensajeria(String  nroMensajeria) {
        ...
        List<DestiDocumentoEnvMensajeriaBean> list = null;
         ...
         sql.append("select ROW_NUMBER() OVER(ORDER BY  a.nu_ann )   fila, a.nu_ann,a.nu_emi,a.nu_des,a.co_loc_des co_local, substring(IDOSGD.PK_SGD_DESCRIPCION_DE_LOCAL(a.co_loc_des), 1, 100)  de_local, \n" +
"                    B.co_dep_emi co_dependencia,\n" +
"                     substring(IDOSGD.PK_SGD_DESCRIPCION_DE_DEPENDENCIA(B.co_dep_emi), 1, 100)  de_dependencia, \n" +
"                    IDOSGD.PK_SGD_DESCRIPCION_DE_DOCUMENTO(B.CO_TIP_DOC_ADM) documento,\n" +
" CASE A.TI_DES WHEN '01'  THEN IDOSGD.PK_SGD_DESCRIPCION_DE_DEPENDENCIA(A.co_dep_DES) +'-'+IDOSGD.PK_SGD_DESCRIPCION_DE_NOM_EMP (A.co_emp_des) "+                 
" WHEN '02' THEN IDOSGD.PK_SGD_DESCRIPCION_DE_PROVEEDOR (A.NU_RUC_DES) +'-'+   "+               
" ( CASE A.REMI_TI_EMI WHEN '03' THEN IDOSGD.PK_SGD_DESCRIPCION_ANI_SIMIL (A.REMI_NU_DNI_EMI) WHEN '04' THEN IDOSGD.PK_SGD_DESCRIPCION_OTRO_ORIGEN (A.REMI_CO_OTR_ORI_EMI)  END )"+
" WHEN '03' THEN IDOSGD.PK_SGD_DESCRIPCION_ANI_SIMIL (A.NU_DNI_DES) "+                                  
" WHEN '04' THEN IDOSGD.PK_SGD_DESCRIPCION_OTRO_ORIGEN (A.CO_OTR_ORI_DES) "+                                  
" WHEN '05' THEN IDOSGD.PK_SGD_DESCRIPCION_DE_DEPENDENCIA(A.co_dep_DES) +'-'+IDOSGD.PK_SGD_DESCRIPCION_DE_NOM_EMP (A.CO_EMP_DES) "+                                  
" END  destinatario, "+     
...
"                    a.ti_des co_tipo_destino,\n" +
"                      a.CDIR_REMITE direccion,\n" +
" CASE A.TI_DES WHEN '01' THEN RTRIM(DEP.NODEP)+'/'+RTRIM(DEP.NOPRV)+'/'+RTRIM(DEP.NODIS)  "+                 
"  WHEN '02' THEN RTRIM(UBIDESTINO.NODEP)+'/'+RTRIM(UBIDESTINO.NOPRV)+'/'+RTRIM(UBIDESTINO.NODIS) "+                 
"  WHEN '03' THEN RTRIM(UBIDESTINO.NODEP)+'/'+RTRIM(UBIDESTINO.NOPRV)+'/'+RTRIM(UBIDESTINO.NODIS) "+                 
"  WHEN '04' THEN RTRIM(UBIDESTINO.NODEP)+'/'+RTRIM(UBIDESTINO.NOPRV)+'/'+RTRIM(UBIDESTINO.NODIS) "+                 
"  WHEN '05' THEN RTRIM(DEP.NODEP)+'/'+RTRIM(DEP.NOPRV)+'/'+RTRIM(DEP.NODIS)  END  departamento,CCOD_TIPO_UBI AS ambito, "+                                  
" CASE B.TI_EMI WHEN '01' THEN  B.NU_DOC_EMI + '-' + B.NU_ANN + '/' + B.DE_DOC_SIG "+
"    WHEN '05' THEN B.NU_DOC_EMI + '-' + B.NU_ANN + '/' + B.DE_DOC_SIG "+
"    ELSE  B.DE_DOC_SIG END NU_DOC  , "+ 
"    CONVERT(varchar(20), B.FE_EMI, 103)  FE_EMI_CORTA,M.FEC_ENVIOMSJ, a.DE_CARGO,M.NU_SER_MSJ+'-'+M.AN_SER_MSJ guia, M.DE_AMBITO ambito \n" +
"                    FROM IDOSGD.tdtv_destinos a  WITH (NOLOCK) INNER JOIN IDOSGD.TDTV_REMITOS B  WITH (NOLOCK) ON a.NU_ANN = B.NU_ANN AND a.NU_EMI = B.NU_EMI\n" +
          "          INNER JOIN IDOSGD.TD_MENSAJERIA M  WITH (NOLOCK) ON M.NU_MSJ=a.NU_MSJ \n"+     
"                    LEFT JOIN ( SELECT D.CO_DEPENDENCIA,D.DE_DEPENDENCIA,D.DE_CORTA_DEPEN,L.DE_NOMBRE_LOCAL,L.DE_DIRECCION_LOCAL,NODEP,NOPRV,NODIS ,L.CCOD_LOCAL\n" +
"                                FROM IDOSGD.RHTM_DEPENDENCIA D WITH (NOLOCK)  \n" +
"                                INNER JOIN IDOSGD.SITM_LOCAL_DEPENDENCIA DL WITH (NOLOCK)  ON DL.CO_DEP=D.CO_DEPENDENCIA\n" +
"                                INNER JOIN IDOSGD.SI_MAE_LOCAL L  WITH (NOLOCK) ON L.CCOD_LOCAL=DL.CO_LOC\n" +
"                                LEFT JOIN IDOSGD.IDTUBIAS U WITH (NOLOCK)  ON U.UBDEP+U.UBPRV+U.UBDIS =L.CO_UBIGEO  ) DEP ON DEP.CO_DEPENDENCIA=A.co_dep_DES AND DEP.CCOD_LOCAL=a.co_loc_des " +
    ...
    public List<DocumentoRecepMensajeriaBean> getLstDetalleMesaVirtual(DestinoResBen oDestinoResBen) {
        StringBuffer sql = new StringBuffer();
        List<DocumentoRecepMensajeriaBean> list = null;
        sql.append("SELECT A.SIDEMIEXT sidEmiExt,'MESA DE PARTES VIRTUAL' detipmsj,ISNULL(CONVERT(NVARCHAR,A.DFECENV,103),' ') fecEnviomsj,\n" +
                   "IDOSGD.PK_SGD_DESCRIPCION_ESTADOS (A.CFLGEST,'IOTDTC_DESPACHO') docEstadoMsj,\n" +
                    "ISNULL(CONVERT(NVARCHAR,A.DFECREGSTDREC,103),' ')  fecRecepmp,\n" +
                    "'cargo' archivo_Cargo  \n" +
                    "FROM IDOSGD.IOTDTC_DESPACHO A  WITH (NOLOCK) "+

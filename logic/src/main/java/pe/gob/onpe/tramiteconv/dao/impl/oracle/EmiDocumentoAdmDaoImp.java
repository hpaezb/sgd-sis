...
/*interoperabilidad*/
        ...
        if (pTipoBusqueda.equals("0") || bBusqFiltro) {
           ...
           /* if (buscarDocumentoEmi.getsTrabDestino()!= null && buscarDocumentoEmi.getsTrabDestino().trim().length() > 0) {
            ...
            if (buscarDocumentoEmi.getCoEstMensajeria()!= null && buscarDocumentoEmi.getCoEstMensajeria().trim().length()>0) {
                sql.append(" AND CO_TIP_DOC_ADM in (SELECT CDOC_TIPDOC FROM IDOSGD.SI_MAE_TIPO_DOC WHERE IN_DOC_SALIDA='1') AND A.ES_DOC_EMI=0 AND  NVL((A.TI_ENV_MSJ),3)= :pcoEstMensajeria");
    ...
    public ProcessResult<List<DocumentoBean>> getDocumentosBuscaEmiAdmList(BuscarDocumentoEmiBean buscarDocumentoEmi,FiltroPaginate paginate) {
        ...
        sql.append(" PK_SGD_DESCRIPCION.ESTADOS(X.ES_DOC_EMI,'TDTV_REMITOS') DE_ES_DOC_EMI ");        
        //sql.append("ROWNUM");
         sql.append(" FROM ( SELECT X.*,ROWNUM AS fila, COUNT(ROWNUM) OVER()  AS filasTotal   ");
        sql.append(" FROM ( ");
        sql.append(" SELECT ");
        sql.append(" A.NU_COR_EMI,A.ti_emi,A.FE_EMI,DECODE(NVL(B.TI_EMI_REF,'0')||NVL(B.IN_EXISTE_ANEXO,'2'),'00',0,'02',0,1) EXISTE_ANEXO, NVL(B.CO_PRIORIDAD,'1') CO_PRI,");
        sql.append(" TO_CHAR(A.FE_EMI,'DD/MM/YYYY HH:MI') FE_EMI_CORTA,");
        sql.append(" A.OBS_DOC DE_OBS_DOC,");
        sql.append(" B.NU_DOC,UPPER(A.DE_ASU) DE_ASU_M,A.NU_ANN,B.NU_EXPEDIENTE,A.NU_EMI,A.TI_CAP,B.IN_EXISTE_DOC EXISTE_DOC,");
        sql.append(" A.ES_DOC_EMI,A.CO_TIP_DOC_ADM,A.NU_CANDES,A.CO_EMP_RES,NVL(A.DOC_ESTADO_MSJ,-1) as DOC_ESTADO_MSJ,");
        sql.append(" (SELECT MAX(C.TI_DES) FROM TDTV_DESTINOS C WHERE C.NU_EMI=A.NU_EMI AND C.NU_ANN=A.NU_ANN   ) as tiDest,NVL(A.TI_ENV_MSJ,-1) tiEnvMsj,");//ORDER BY C.TI_DES DESC INTEROPERABILIDAD
        ...
        sql.append(" FROM TDTV_REMITOS A INNER JOIN TDTX_REMITOS_RESUMEN B ON B.NU_ANN=A.NU_ANN  AND B.NU_EMI=A.NU_EMI  ");
        ...
        //YUAL
      /*  if(buscarDocumentoEmi.getsEstadoDoc().equals("7"))
        {
        sql.append("  AND ( ((SELECT co_empleado FROM RHTM_DEPENDENCIA WHERE co_dependencia=:pCoDepEmi)='"+buscarDocumentoEmi.getsCoEmpleado()+"' AND A.CO_EMP_EMI ='"+buscarDocumentoEmi.getsCoEmpleado()+"' ) OR ( (SELECT co_empleado FROM RHTM_DEPENDENCIA WHERE co_dependencia=:pCoDepEmi)!='"+buscarDocumentoEmi.getsCoEmpleado()+"' AND A.CO_DEP_EMI = :pCoDepEmi)  ) ");
        }*/
        // Parametros Basicos
        objectParam.put("pCoDepEmi", buscarDocumentoEmi.getsCoDependencia());
       ...
       // }
        ...
        if(bExcepSeach=="0") 
        {
           ...
           /*
            ...
            //YUAL
            if (buscarDocumentoEmi.getCoEstMensajeria()!= null && buscarDocumentoEmi.getCoEstMensajeria().trim().length()>0) {
                sql.append(" AND CO_TIP_DOC_ADM in ( SELECT CDOC_TIPDOC FROM SI_MAE_TIPO_DOC WHERE IN_DOC_SALIDA=1 )  AND A.ES_DOC_EMI=0 AND  NVL((A.TI_ENV_MSJ),3)= :pcoEstMensajeria");
                objectParam.put("pcoEstMensajeria", buscarDocumentoEmi.getCoEstMensajeria());
            }
	...
	sql.append(") A ");
        ...
        try {
            list = this.jdbcTemplate.query(sql.toString(), BeanPropertyRowMapper.newInstance(DocumentoBean.class),
                    new Object[]{pannio,pcoDepen, pcoDepen, ptiDoc, ptiDoc, pannio,pcoDepen});
        } catch (EmptyResultDataAccessException e) {
            list = null;
        } catch (Exception e) {
            list = null;
    ...
    */
    ...
    public List<DocumentoBean> getLstDocRecepcionadoRef(String pcoDepen, String pannio, String ptiDoc, String pnuDoc) {
        ...
        List<DocumentoBean> list1 = new ArrayList<DocumentoBean>();
        ...
        sql1.append("SELECT \n"+
                    ...
                    "from TDTV_REMITOS R,TDTV_DESTINOS D,TDTX_REMITOS_RESUMEN RR\n" +
                    "where\n " +
                    "rownum<=200 and r.es_eli = '0'\n" +
                    "AND d.es_eli = '0'\n" +
                    "AND r.nu_ann = ");
                    sql1.append(pannio);
                    sql1.append(" and rr.nu_ann=r.nu_ann\n" +
                    "AND rr.nu_emi = r.nu_emi\n" +
                    "AND d.nu_ann = r.nu_ann\n" +
                    "AND d.nu_emi = r.nu_emi\n" +
                    "AND r.es_doc_emi NOT IN ('5', '9', '7')\n" +
             ...
             sql1.append("'\n");
                    if(pnuDoc!=null&&pnuDoc.trim().length()>0){
                        pnuDoc=Utility.getInstancia().rellenaCerosIzquierda(pnuDoc, 6);
                        sql1.append("AND (R.NU_DOC_EMI = '").append(pnuDoc).append("' OR RR.NU_EXPEDIENTE LIKE '%").append(pnuDoc).append("%')\n");
                    }
...
//                    new Object[]{pannio,pcoDepen, pcoDepen, ptiDoc});
            list1 = this.jdbcTemplate.query(sql1.toString(), BeanPropertyRowMapper.newInstance(DocumentoBean.class));
...
//                    ",A.NU_COR_DOC='"+ documentoEmiBean.getNuCorDoc()+ "',\n");
    ...
    public String insDocumentoEmiBean(DocumentoEmiBean documentoEmiBean, ExpedienteBean expedienteBean, RemitenteEmiBean remitenteEmiBean) {
        ...
        try {
            ...
            // if (documentoEmiBean.getConfidencial()==null || documentoEmiBean.getConfidencial().trim().equals("") ){                               
            //   documentoEmiBean.setConfidencial("0");
           // }
            ...
            this.jdbcTemplate.update(sqlUpd.toString(), new Object[]{documentoEmiBean.getNuAnn(), snuEmi, snuCorEmi,
                ...
                });//,documentoEmiBean.getConfidencial()
...
//                + "NU_DES=?");
    ...
    public String insExpedienteBean(ExpedienteBean expedienteBean) {
        ...
        sqlQry1.append("select lpad(rtrim(ltrim(to_char(NVL(max(NU_CORR_EXP),0) +1 ))), 7, '0') nuCorrExp, \n"
                ...
                + " PK_SGD_DESCRIPCION.DE_SIGLA_CORTA(?) deSiglaCorta\n"
 ...
 @Override
    public String updDocumentoLimpiarObj(DocumentoEmiBean documentoEmiBean) {
        ...
        sqlUpd.append("UPDATE IDOSGD.TDTV_ARCHIVO_DOC SET BL_DOC = NULL, DE_RUTA_ORIGEN = NULL, FEULA=TO_CHAR(SYSDATE,'YYYYMMDD') \n"
                    + "WHERE NU_ANN = ? \n"
                    + "AND NU_EMI = ? ");
    ...
    public String updDocumentoObj(final DocumentoObjBean docObjBean) {
        ...
        if (inInsert) {
            ...
            try {
                    ...
                    {
                    sql1.append("UPDATE TDTV_ARCHIVO_DOC SET W_BL_DOC = ?, W_DE_RUTA_ORIGEN = ? \n"
                    + "WHERE NU_ANN = ? \n"
                    + "AND NU_EMI = ? ");
...
//                this.jdbcTemplate.update(sql1.toString(), new Object[]{docObjBean.getNuAnn(), docObjBean.getNuEmi()});            
    ...
    public String getUsuarioNofirmaProveido(String pcoEmpleado) {
        ...
        sql.append(" SELECT COUNT(*) TOTAL FROM SI_ELEMENTO WHERE CTAB_CODTAB='NO_FIRMA_PROVEIDO' AND CELE_DESELE='"+pcoEmpleado+"' ");
        try {
    ...
    public String updDocumentoEmiAdmBean(String nuAnn, String nuEmi, DocumentoEmiBean documentoEmiBean, ExpedienteBean expedienteBean, RemitenteEmiBean remitenteEmiBean, String pcoUserMod) {
        ...
        if (documentoEmiBean != null) {
           ...
           /* if (documentoEmiBean.getConfidencial()==null || documentoEmiBean.getConfidencial().trim().equals("") ){                 
                 sqlUpd.append("A.CONFIDENCIAL='0',\n");  
            }
            else {
                 sqlUpd.append("A.CONFIDENCIAL='" + documentoEmiBean.getConfidencial()+ "',\n");
    ...
    public String getNumeroDocSiguienteAdm(String pnuAnn, String pcoDepEmi, String pcoDoc) {
        ...
        try {
            ...
            sql.append("         SELECT trim(to_char(nvl(MAX(nu_doc_emi), 0) + 1)) \n"
                    ...
                    + "            AND tr.ti_emi = '01'\n"
                    + "            AND tr.nu_cor_doc = 1\n"
                    //yual antes  + "            AND tr.es_doc_emi != '9'\n"
                    //+ "            AND tr.es_doc_emi != '9'\n"
...
//            vReturn = "OK";
    ...
    public List<DestinatarioDocumentoEmiBean> getLstDestExternoGrupo(String pcoGrupo,  String pTiDes) {
        ...
        if(pTiDes.equals("02"))
        {
            ...
            sql.append("SELECT CPRO_RUC NURUC, CPRO_RAZSOC deProveedor,NVL(CPRO_DOMICIL,' ') cdirRemite,NVL(CPRO_TELEFO,' ') cTelefono,NVL(CPRO_EMAIL,' ') cexpCorreoe, \n" +
                        "NVL(TRIM(NODEP)||' '||TRIM(NOPRV)||' '||TRIM(NODIS),' ') UBIGEO, NVL(TRIM(CUBI_CODDEP),' ') CCOD_DPTO,NVL(TRIM(CUBI_CODPRO),' ') CCOD_PROV,NVL(TRIM(CUBI_CODDIS),' ') CCOD_DIST\n" +
                        "FROM LG_PRO_PROVEEDOR J LEFT JOIN IDTUBIAS U ON J.CUBI_CODDEP||J.CUBI_CODPRO||J.CUBI_CODDIS=U.UBDEP||U.UBPRV||U.UBDIS \n" +
                        "WHERE J.CPRO_RUC IN (SELECT CO_DES FROM TDTD_DEP_GRU_VAR WHERE CO_GRU_DES = ? AND Es_dep_gru = '1' AND TI_DES = ?)\n" +
                        "order by J.rowid");
        }

        if(pTiDes.equals("03"))
        {
            sql.append("SELECT C.NULEM nu_dni,NVL(substr(PK_SGD_DESCRIPCION.ANI_SIMIL(C.NULEM), 1, 100),' ') de_ciudadano,\n"+
            "NVL(C.DEDOMICIL,' ') CDIR_REMITE,NVL(C.DEEMAIL,' ') CEXP_CORREOE,NVL(C.DETELEFO,' ') CTELEFONO,NVL(C.UBDEP,' ') CCOD_DPTO,NVL(C.UBPRV,' ') CCOD_PROV,NVL(C.UBDIS,' ') CCOD_DIST,\n" +
            "NVL(TRIM(NODEP)||' '||TRIM(NOPRV)||' '||TRIM(NODIS),' ') UBIGEO\n" +          
            "FROM TDTX_ANI_SIMIL C LEFT JOIN IDTUBIAS U ON C.UBDEP||C.UBPRV||C.UBDIS=U.UBDEP||U.UBPRV||U.UBDIS \n" +
            "WHERE C.NULEM IN (SELECT CO_DES FROM TDTD_DEP_GRU_VAR WHERE CO_GRU_DES = ? AND Es_dep_gru = '1' AND TI_DES = ?)\n" +
            "order by C.rowid ");       
        }
        ...
        if(pTiDes.equals("04"))
        {
            sql.append( "SELECT O.CO_OTR_ORI co_otro_origen,O.DE_APE_PAT_OTR||' '||O.DE_APE_MAT_OTR||', '||O.DE_NOM_OTR || ' - ' ||\n" +
                        "O.DE_RAZ_SOC_OTR ||'##'||  NVL(S.CELE_DESELE,'   ') ||'##'||  O.NU_DOC_OTR_ORI  de_otro_origen_full\n" +
                        "NVL(O.DE_DIR_OTRO_ORI,' ') CDIR_REMITE,NVL(O.DE_EMAIL,' ') CEXP_CORREOE,NVL(O.DE_TELEFO,' ') CTELEFONO,NVL(O.UB_DEP,' ') CCOD_DPTO,NVL(O.UB_PRO,' ') CCOD_PROV,NVL(O.UB_DIS,' ') CCOD_DIST,\n" +
                        "NVL(TRIM(NODEP)||' '||TRIM(NOPRV)||' '||TRIM(NODIS),' ') UBIGEO\n" +          
                        "FROM TDTR_OTRO_ORIGEN O LEFT JOIN IDTUBIAS U ON O.UB_DEP||O.UB_PRO||O.UB_DIS=U.UBDEP||U.UBPRV||U.UBDIS \n" +
                        "LEFT JOIN  (\n" +
                        "SELECT CELE_CODELE, CELE_DESELE  FROM SI_ELEMENTO  WHERE CTAB_CODTAB ='TIP_DOC_IDENT') S\n" +
                        "ON O.CO_TIP_OTR_ORI = S.CELE_CODELE \n" +                    
                        "WHERE O.CO_OTR_ORI IN (SELECT CO_DES FROM TDTD_DEP_GRU_VAR WHERE CO_GRU_DES = ? AND Es_dep_gru = '1' AND TI_DES = ?)\n" +
                        "order by O.rowid ");    
        }

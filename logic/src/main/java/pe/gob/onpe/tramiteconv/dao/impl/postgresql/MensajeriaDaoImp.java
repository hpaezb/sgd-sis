...
public class MensajeriaDaoImp extends SimpleJdbcDaoBase  implements MensajeriaDao{
    ...
    public List<DocumentoRecepMensajeriaBean> getDocumentoRecepMensajeria(BuscarDocumentoRecepMensajeriaBean buscarDocumentoRecepMensajeriaBean) {
        ...
        sql.append("SELECT  NU_ANN,NU_EMI,\n" +
                ...
                "            DE_ASU,\n" +
                " (CASE WHEN X.TI_EMI='01' THEN  X.NU_DOC_EMI || '-' || X.NU_ANN || '/' || X.DE_DOC_SIG  "+
                "      WHEN X.TI_EMI='05' THEN  X.NU_DOC_EMI || '-' || X.NU_ANN || '/' || X.DE_DOC_SIG END ) NU_DOC, "+
                " (CASE WHEN X.NU_CANDES='1' THEN  IDOSGD.PK_SGD_DESCRIPCION_TI_DES_EMP (X.NU_ANN, X.NU_EMI)  "+
                "      ELSE  IDOSGD.PK_SGD_DESCRIPCION_TI_DES_EMP_V (X.NU_ANN, X.NU_EMI) END ) DE_EMP_DES, "+                 
                ...
                "         A.CO_DEP,COALESCE(TOTAL_DESTINO,0)::text TOTAL_DESTINO , COALESCE(TOTAL_ENVIADO,0)::text TOTAL_ENVIADO , COALESCE(TOTAL_PENDIENTE,0)::text TOTAL_PENDIENTE \n" +
                "        ,FEC_ENVIOMSJ, DE_AMBITO,DE_TIP_ENV,NU_SER_MSJ, FE_PLA_MSJ \n"+
                "        ,  C.DIAS_PLA_DEVO,C.DIAS_PLA_ENTR,A.FE_ENV_MES \n"+
                "          FROM IDOSGD.TDTV_REMITOS A INNER JOIN IDOSGD.TDTX_REMITOS_RESUMEN B ON A.NU_ANN = B.NU_ANN AND A.NU_EMI = B.NU_EMI\n" +
                "        LEFT JOIN (SELECT NU_ANN,NU_EMI, COALESCE(COUNT(NU_DES),0) TOTAL_DESTINO FROM IDOSGD.TDTV_DESTINOS WHERE TI_DES<>'01' AND ES_DOC_REC IN  ('0','1','2','3','4') GROUP BY NU_ANN,NU_EMI ) E ON A.NU_ANN = E.NU_ANN AND A.NU_EMI = E.NU_EMI\n" +
                "        LEFT JOIN (SELECT NU_ANN,NU_EMI,COALESCE(COUNT(NU_DES),0) TOTAL_ENVIADO ,M.FEC_ENVIOMSJ,M.DIAS_PLA_DEVO,M.DIAS_PLA_ENTR, M.DE_AMBITO,M.DE_TIP_ENV,M.NU_SER_MSJ||'-'||M.AN_SER_MSJ NU_SER_MSJ, M.FE_PLA_MSJ\n" +
                "                   FROM IDOSGD.TDTV_DESTINOS D LEFT JOIN IDOSGD.TD_MENSAJERIA M ON M.NU_MSJ=D.NU_MSJ\n" +
                ...
                "        LEFT JOIN (SELECT NU_ANN,NU_EMI, COALESCE(COUNT(NU_DES),0) TOTAL_PENDIENTE FROM IDOSGD.TDTV_DESTINOS WHERE TI_DES <>'01' AND ES_DOC_REC IN ('0','1') GROUP BY NU_ANN,NU_EMI) D ON A.NU_ANN = D.NU_ANN AND A.NU_EMI = D.NU_EMI\n" +
                "     LEFT JOIN (SELECT CO_EST,DE_EST DE_ES_DOC_EMI_MP FROM IDOSGD.TDTR_ESTADOS WHERE DE_TAB='ENVIO_DOCUMENTO_MSJ') EST ON EST.CO_EST= A.DOC_ESTADO_MSJ \n"+
...
//        sql.append(" AND A.ES_DOC_EMI<>9");
        ...
        if (pTipoBusqueda.equals("0") || bBusqFiltro) {
            ...
            if (buscarDocumentoRecepMensajeriaBean.getEsFiltroFechaEnvMsj()!= null  && (pEsFiltroFechaENVmSJ.equals("1") || pEsFiltroFechaENVmSJ.equals("3"))) {
                ...
                if (vFeEmiIni != null && vFeEmiIni.trim().length() > 0  && vFeEmiFin != null && vFeEmiFin.trim().length() > 0) {
                    sql.append(" AND A.FE_ENV_MES between TO_DATE(:pFeEmiIniEnv,'DD/MM/YYYY') AND TO_DATE(:pFeEmiFinEnv,'DD/MM/YYYY') + TIME '23:59:59' ");
      ...
      //  }        
          if (buscarDocumentoRecepMensajeriaBean.getBusDesti()!= null && buscarDocumentoRecepMensajeriaBean.getBusDesti().trim().length() > 0  ) {
                sql.append("  AND UPPER(CASE WHEN A.NU_CANDES='1' ,\n" +
                        " THEN IDOSGD.PK_SGD_DESCRIPCION_TI_DES_EMP (A.NU_ANN, A.NU_EMI) \n" +
                        " ELSE IDOSGD.PK_SGD_DESCRIPCION.TI_DES_EMP_V (A.NU_ANN, A.NU_EMI)\n" +
                        " END )  LIKE '%'||UPPER(:pDesti)||'%' ");
    ...
    public List<DestiDocumentoEnvMensajeriaBean> getLstDestinoEnvMensajeria(String pnuAnnpnuEmi) {
        ...
        sql.append("select row_number() OVER (ORDER BY 1) AS fila, a.nu_ann,a.nu_emi,a.nu_des,a.co_loc_des co_local,COALESCE(a.co_loc_des,substr(IDOSGD.PK_SGD_DESCRIPCION_DE_LOCAL(a.co_loc_des), 1, 100),NULL) de_local, \n" +
"                    B.co_dep_emi co_dependencia,\n" +
"                    COALESCE(substr(IDOSGD.PK_SGD_DESCRIPCION_DE_DEPENDENCIA(B.co_dep_emi), 1, 100),NULL) de_dependencia, \n" + 
"                    IDOSGD.PK_SGD_DESCRIPCION_DE_DOCUMENTO(B.CO_TIP_DOC_ADM) documento,\n" +
"                    (CASE A.TI_DES \n" +
"                            WHEN '01' THEN IDOSGD.PK_SGD_DESCRIPCION_DE_DEPENDENCIA(A.co_dep_DES) ||'-'||IDOSGD.PK_SGD_DESCRIPCION_DE_NOM_EMP (A.co_emp_des) \n" +
...
"                            END)  destinatario,\n" +
"                    a.co_mot co_tramite,COALESCE(A.co_mot,substr(IDOSGD.PK_SGD_DESCRIPCION_MOTIVO(a.co_mot), 1, 100),NULL) de_tramite, \n" +
"                    a.co_pri co_prioridad, \n" +
"                    a.de_pro de_indicaciones, \n" +
"                    a.ti_des co_tipo_destino,\n" +
"                      a.CDIR_REMITE direccion,\n" +
"                      (CASE A.TI_DES \n" +
"                            WHEN '01' THEN TRIM(DEP.NODEP)||'/'||TRIM(DEP.NOPRV)||'/'||TRIM(DEP.NODIS) \n" +
"                            WHEN '02' THEN TRIM(UBIDESTINO.NODEP)||'/'||TRIM(UBIDESTINO.NOPRV)||'/'||TRIM(UBIDESTINO.NODIS) \n" +
"                            WHEN '03' THEN TRIM(UBIDESTINO.NODEP)||'/'||TRIM(UBIDESTINO.NOPRV)||'/'||TRIM(UBIDESTINO.NODIS) \n" +
"                            WHEN '04' THEN TRIM(UBIDESTINO.NODEP)||'/'||TRIM(UBIDESTINO.NOPRV)||'/'||TRIM(UBIDESTINO.NODIS) \n" +
"                            WHEN '05' THEN TRIM(DEP.NODEP)||'/'||TRIM(DEP.NOPRV)||'/'||TRIM(DEP.NODIS)\n" +
"                            END ) departamento,CCOD_TIPO_UBI AS ambito,\n" +
"                  (CASE B.TI_EMI  \n" +
"                                WHEN '01' THEN B.NU_DOC_EMI || '-' || B.NU_ANN || '/' || B.DE_DOC_SIG  \n" +
"                                WHEN '05' THEN B.NU_DOC_EMI || '-' || B.NU_ANN || '/' || B.DE_DOC_SIG  \n" +
"                                ELSE  B.DE_DOC_SIG END) NU_DOC,TO_CHAR(B.FE_EMI,'DD/MM/YYYY') FE_EMI_CORTA,a.CCOD_DPTO,a.CCOD_PROV,a.CCOD_DIST \n" +
"                    FROM IDOSGD.tdtv_destinos a INNER JOIN IDOSGD.TDTV_REMITOS B ON a.NU_ANN = B.NU_ANN AND a.NU_EMI = B.NU_EMI\n" +
...
"                                FROM IDOSGD.RHTM_DEPENDENCIA D \n" +
"                                INNER JOIN IDOSGD.SITM_LOCAL_DEPENDENCIA DL ON DL.CO_DEP=D.CO_DEPENDENCIA\n" +
"                                INNER JOIN IDOSGD.SI_MAE_LOCAL L ON L.CCOD_LOCAL=DL.CO_LOC\n" +
"                                LEFT JOIN IDOSGD.IDTUBIAS U ON U.UBDEP||U.UBPRV||U.UBDIS =L.CO_UBIGEO  ) DEP ON DEP.CO_DEPENDENCIA=A.co_dep_DES AND DEP.CCOD_LOCAL=a.co_loc_des " +
                " LEFT JOIN IDOSGD.IDTUBIAS UBIDESTINO ON UBIDESTINO.UBDEP||UBIDESTINO.UBPRV||UBIDESTINO.UBDIS =a.CCOD_DPTO||a.CCOD_PROV||a.CCOD_DIST  \n"+
    ...
    public String insMensajeriaDocumento(DocumentoRecepMensajeriaBean documentoMensajeria) {
        ...
        sqlQry1.append("select NEXTVAL('idosgd.sec_td_mensajeria')"); 
        sqlQry2.append("select to_char(now(),'dd/mm/yyyy hh24:mi:ss')  ");        
        ...
        sqlIns.append("INSERT INTO  idosgd.td_mensajeria ( nu_msj,fe_reg_msj,co_use_cre,de_ambito,de_tip_msj,re_env_msj,\n" +
                    ...
                    "    VALUES(cast(? as int),(select TO_TIMESTAMP(?,'dd/mm/yyyy hh24:mi:ss')  ),?,?,?,?,?,?,?,(select TO_TIMESTAMP(?,'dd/mm/yyyy hh24:mi:ss') ),(select TO_TIMESTAMP(?,'dd/mm/yyyy hh24:mi:ss')  ),(select TO_TIMESTAMP(?,'dd/mm/yyyy hh24:mi:ss')  ),(select TO_TIMESTAMP(?,'dd/mm/yyyy hh24:mi:ss')  ),'0',?,?,?,?,?)");
...
//                    "    VALUES(?,(select to_date(?,'dd/mm/yyyy hh24:mi:ss') from dual),?,?,?,?,?,?,?,(select to_date(?,'dd/mm/yyyy hh24:mi:ss') from dual),(select to_date(?,'dd/mm/yyyy hh24:mi:ss') from dual),(select to_date(?,'dd/mm/yyyy hh24:mi:ss') from dual),(select to_date(?,'dd/mm/yyyy hh24:mi:ss') from dual),'0',?,?)");
        ...
        try{
            ...
            sqlUpd.append("UPDATE idosgd.TDTV_DESTINOS SET ES_DOC_REC='2', NU_MSJ='"+snuNumsj+"', FEC_ENVIOMSJ= now()  WHERE  NU_ANN||NU_EMI||NU_DES in ( "+ documentoMensajeria.getCodigo()+" ) ");
            sqlUpdRemito.append("UPDATE idosgd.TDTV_REMITOS  SET  FEC_ENVIODEP=now(),DOC_ESTADO_MSJ='2' \n" +
                    " WHERE  NU_ANN||NU_EMI IN ( SELECT NU_ANN||NU_EMI FROM  idosgd.TDTV_DESTINOS WHERE NU_ANN||NU_EMI||NU_DES in (  "+documentoMensajeria.getCodigo()+"  ) ) ");
            ...
            sqlUpdRemitoParcial.append("UPDATE idosgd.TDTV_REMITOS  SET  FEC_ENVIODEP=now(),DOC_ESTADO_MSJ='3' \n" +
"                     WHERE  NU_ANN||NU_EMI IN (SELECT NU_ANN||NU_EMI FROM  idosgd.TDTV_DESTINOS WHERE ES_DOC_REC<>'2' \n" +
"                     and NU_ANN||NU_EMI in ( SELECT NU_ANN||NU_EMI FROM  idosgd.TDTV_DESTINOS WHERE   NU_ANN||NU_EMI||NU_DES in (  "+documentoMensajeria.getCodigo()+"  )  ) )");
...
//                documentoMensajeria.getHoenvmsj(),documentoMensajeria.getFeplamsj(),documentoMensajeria.getHoplamsj(),documentoMensajeria.getDiasEntrega(),documentoMensajeria.getDiasDevoluvion()});
    ...
    public String updMensajeriaDocumentoRecibir(String codigo) {
        ...
        StringBuffer sqlIns = new StringBuffer(); 
        StringBuffer sqlUpd = new StringBuffer(); 
        sqlIns.append("UPDATE idosgd.TDTV_REMITOS  SET FEC_RECEPMP=now(),DOC_ESTADO_MSJ='1'  WHERE  NU_ANN||NU_EMI IN ("+codigo+" ) ");
    ...
    public String selectCalcularFechaPlazo(DocumentoRecepMensajeriaBean documentoMensajeria) {
        ...
        StringBuffer sqlQry1 = new StringBuffer();
        StringBuffer sqlQry2 = new StringBuffer();
        sqlQry0.append("SELECT NELE_NUMSEC::text FROM idosgd.SI_ELEMENTO WHERE CTAB_CODTAB='DE_TIP_MSJ_MENS' AND CELE_DESELE='"+documentoMensajeria.getDetipmsj()+"'");  
        ...
        try{
            ...
            if(confCalculaPena.equals("1")){
                ...
                sqlQry2.append("select  idosgd.pk_sgd_consultas_calcular_dias_habiles(TO_DATE('"+documentoMensajeria.getFecenviomsj()+"','DD/MM/YYYY'),"+sdias+") ");  
                String snufecha = this.jdbcTemplate.queryForObject(sqlQry2.toString(), String.class); 
    ...
    public List<DocumentoRecepMensajeriaBean> getLstDetalleMensajeria(DestinoResBen oDestinoResBen) {
        ...
        sql.append("SELECT M.NU_MSJ::text as numsj,M.DE_TIP_MSJ as detipmsj,\n" +
                    ...
                    "  B.NU_COR_EMI::text||'-'|| D.NU_DES::text||'-1' as nu_Acta_Vis1, B.NU_COR_EMI::text||'-'|| D.NU_DES ::text||'-2' as nu_Acta_Vis2, "+
                    "  COALESCE(TO_CHAR(DM1.FEC_NOT_ACTA,'DD/MM/YYYY'),' ') as fe_Acta_Vis1,COALESCE(DM1.ES_NOT_ACTA,'2') as es_Acta1_msj, "+
                    "  COALESCE(TO_CHAR(DM2.FEC_NOT_ACTA,'DD/MM/YYYY'),' ') as fe_Acta_Vis2,COALESCE(DM2.ES_NOT_ACTA,'2') as es_Acta2_msj, "+
                    " COALESCE(DM1.DE_RUT_ORI,' ') as archivo_Acta1, "+
                    " COALESCE(DM2.DE_RUT_ORI,' ') as archivo_Acta2, "+
                    " COALESCE(CARGO.NU_ANE::text,' ') as archivo_Cargo  "+
                    " FROM idosgd.TDTV_DESTINOS D \n" +
                    " INNER JOIN idosgd.TDTV_REMITOS B ON D.NU_ANN = B.NU_ANN AND D.NU_EMI = B.NU_EMI "+
                    " INNER JOIN idosgd.TD_MENSAJERIA M ON M.NU_MSJ=D.NU_MSJ \n" +
                    " LEFT JOIN idosgd.RHTM_PER_EMPLEADOS R ON R.CEMP_NU_DNI=M.RE_ENV_MSJ \n" +
                    " LEFT JOIN idosgd.LG_PRO_PROVEEDOR P ON P.CPRO_RUC=M.RE_ENV_MSJ "+ 
                    " LEFT JOIN idosgd.TD_DET_MENSAJERIA DM1 ON DM1.NU_ANN=D.NU_ANN AND DM1.NU_EMI=D.NU_EMI AND DM1.NU_DES=D.NU_DES AND DM1.NU_ACTA=B.NU_COR_EMI::text||'-'|| D.NU_DES::text||'-1' "+
                    " LEFT JOIN idosgd.TD_DET_MENSAJERIA DM2 ON DM2.NU_ANN=D.NU_ANN AND DM2.NU_EMI=D.NU_EMI AND DM2.NU_DES=D.NU_DES AND DM2.NU_ACTA=B.NU_COR_EMI::text||'-'|| D.NU_DES::text||'-2' "+
                    " LEFT JOIN idosgd.TDTV_ANEXOS CARGO ON CARGO.NU_ANN=D.NU_ANN AND CARGO.NU_EMI=D.NU_EMI AND CARGO.NU_DES=D.NU_DES AND CARGO.DE_DET='CARGO_NOTIFICACION'|| D.NU_DES::text||'.pdf' "+
...
"                            WHEN '01' THEN idosgd.PK_SGD_DESCRIPCION_DE_DEPENDENCIA(A.co_dep_DES) ||'-'||idosgd.PK_SGD_DESCRIPCION_DE_NOM_EMP (A.co_emp_des) \n" +
"                            WHEN '02' THEN idosgd.PK_SGD_DESCRIPCION_DE_PROVEEDOR (A.NU_RUC_DES) ||'-'|| "
                 + " COALESCE (CASE WHEN A.REMI_TI_EMI='03' THEN idosgd.PK_SGD_DESCRIPCION_ANI_SIMIL (A.REMI_NU_DNI_EMI) WHEN A.REMI_TI_EMI='04' THEN idosgd.PK_SGD_DESCRIPCION_OTRO_ORIGEN (A.REMI_CO_OTR_ORI_EMI) END ,'')  \n" +
"                            WHEN '03' THEN idosgd.PK_SGD_DESCRIPCION_ANI_SIMIL (A.NU_DNI_DES) \n" +
"                            WHEN '04' THEN idosgd.PK_SGD_DESCRIPCION_OTRO_ORIGEN (A.CO_OTR_ORI_DES) \n" +
"                            WHEN '05' THEN idosgd.PK_SGD_DESCRIPCION_DE_DEPENDENCIA(A.co_dep_DES) ||'-'||idosgd.PK_SGD_DESCRIPCION_DE_NOM_EMP (A.CO_EMP_DES) \n" +
"                            END ) destinatario,\n" +
...
"                                WHEN '01' THEN B.NU_DOC_EMI || '-' || B.NU_ANN || '/' || B.DE_DOC_SIG  \n" +
"                                WHEN '05' THEN B.NU_DOC_EMI || '-' || B.NU_ANN || '/' || B.DE_DOC_SIG  \n" +
"                                ELSE B.DE_DOC_SIG END ) NU_DOC  ,TO_CHAR(B.FE_EMI,'DD/MM/YYYY') FE_EMI_CORTA,M.FEC_ENVIOMSJ, a.DE_CARGO,M.NU_SER_MSJ||'-'||M.AN_SER_MSJ guia, M.DE_AMBITO ambito \n" +
"                    FROM idosgd.tdtv_destinos a INNER JOIN idosgd.TDTV_REMITOS B ON a.NU_ANN = B.NU_ANN AND a.NU_EMI = B.NU_EMI\n" +
          "          INNER JOIN idosgd.TD_MENSAJERIA M ON M.NU_MSJ=a.NU_MSJ \n"+     
"                    LEFT JOIN ( SELECT D.CO_DEPENDENCIA,D.DE_DEPENDENCIA,D.DE_CORTA_DEPEN,L.DE_NOMBRE_LOCAL,L.DE_DIRECCION_LOCAL,NODEP,NOPRV,NODIS ,L.CCOD_LOCAL\n" +
"                                FROM idosgd.RHTM_DEPENDENCIA D \n" +
"                                INNER JOIN idosgd.SITM_LOCAL_DEPENDENCIA DL ON DL.CO_DEP=D.CO_DEPENDENCIA\n" +
"                                INNER JOIN idosgd.SI_MAE_LOCAL L ON L.CCOD_LOCAL=DL.CO_LOC\n" +
"                                LEFT JOIN idosgd.IDTUBIAS U ON U.UBDEP||U.UBPRV||U.UBDIS =L.CO_UBIGEO  ) DEP ON DEP.CO_DEPENDENCIA=A.co_dep_DES AND DEP.CCOD_LOCAL=a.co_loc_des " +
    ...
    public List<DocumentoRecepMensajeriaBean> getLstDetalleMesaVirtual(DestinoResBen oDestinoResBen) {
        StringBuffer sql = new StringBuffer();
        List<DocumentoRecepMensajeriaBean> list = null;
        sql.append("SELECT A.SIDEMIEXT::text sidEmiExt,'MESA DE PARTES VIRTUAL' detipmsj,COALESCE(TO_CHAR(A.DFECENV,'DD/MM/YYYY'),' ') fecEnviomsj,\n" +
                   "idosgd.PK_SGD_DESCRIPCION_ESTADOS (A.CFLGEST,'IOTDTC_DESPACHO') docEstadoMsj,\n" +
                    "COALESCE(TO_CHAR(A.DFECREGSTDREC,'DD/MM/YYYY'),' ')  fecRecepmp,\n" +
                    "'cargo' archivo_Cargo  \n" +
                    "FROM idosgd.IOTDTC_DESPACHO A "+

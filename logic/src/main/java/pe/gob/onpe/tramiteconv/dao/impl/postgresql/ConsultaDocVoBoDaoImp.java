...
public class ConsultaDocVoBoDaoImp extends SimpleJdbcDaoBase implements ConsultaDocVoBoDao{
    ...
    public List<DocumentoVoBoBean> getLsDocsVoBo(BuscarDocumentoVoBoBean bDocVoBo) {
        ...
        sql.append("	(SELECT DE_EST FROM IDOSGD.TDTR_ESTADOS WHERE CO_EST || DE_TAB = "); 
        sql.append("CASE WHEN VB.IN_VB='B' THEN '0' ELSE VB.IN_VB  END  || 'TDTV_PERSONAL_VB') DE_ES_DOC_EMI,"); 
        sql.append("	RR.IN_EXISTE_DOC EXISTE_DOC,");
        sql.append(" CASE WHEN (COALESCE(RR.TI_EMI_REF,'0')||COALESCE(RR.IN_EXISTE_ANEXO,'2'))='00' THEN 0"
                + " WHEN (COALESCE(RR.TI_EMI_REF,'0')||COALESCE(RR.IN_EXISTE_ANEXO,'2'))='02' THEN 0 "
                + " ELSE 1 END AS EXISTE_ANEXO ");
        sql.append("	FROM IDOSGD.TDTV_REMITOS RE, IDOSGD.TDTV_PERSONAL_VB VB, IDOSGD.TDTX_REMITOS_RESUMEN RR");
...
//                sql.append(" AND RE.FE_EMI between TO_DATE(:pFeEmiIni,'dd/mm/yyyy') AND TO_DATE(:pFeEmiFin,'dd/mm/yyyy') + 0.99999");
        ...
        if (pTipoBusqueda.equals("1")) {
            ...
            if (bDocVoBo.getAsunto()!= null && bDocVoBo.getAsunto().trim().length() > 1) {
                ...
                sql.append(" AND UPPER(RR.DE_ASU) LIKE UPPER('%'||:pDeAsunto||'%') ");
...
//        sql.append("WHERE ROWNUM < 101");
    ...
    public DocumentoVoBoBean getDocumentoVoBo(String pnuAnn, String pnuEmi, String coDep, String coEmpVb) {
        ...
        sql.append("SELECT \n" +
                    ...
                    " CASE WHEN RE.TI_EMI= '01' THEN COALESCE(RE.NU_DOC_EMI,' ') || '-' || RE.NU_ANN || '-' || RE.DE_DOC_SIG " +
                    " WHEN RE.TI_EMI='05' THEN COALESCE(RE.NU_DOC_EMI,' ') || '-' || RE.NU_ANN || '-' || RE.DE_DOC_SIG " +
                    " ELSE COALESCE(RE.DE_DOC_SIG,'S/N') END AS NU_DOC," +                
                    " CASE WHEN VB.IN_VB = 'B' THEN '0' " +
                    " ELSE VB.IN_VB END ES_DOC_EMI," + 

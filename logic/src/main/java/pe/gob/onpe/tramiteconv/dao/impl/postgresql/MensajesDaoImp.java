  ...
  private SimpleJdbcCall spDescargamsj;
    ...
    public List<EstadoDocumentoBean> listEstadosCarga(String nomTabla) {
        ...
        sql.append("SELECT DE_EST,CO_EST \n" +
                    ...
                    "UNION SELECT 'PENDIENTE' as DE_EST,'2' as CO_EST   "+
                    "ORDER BY CO_EST");
    ...
    public List<MensajesConsulBean> getBuscaDocumentosCarga(BuscarDocumentoCargaMsjBean buscarDocumentoCargaMsjBean) {
        ...
        sql.append("SELECT  ");
        sql.append("A.NU_MSJ,TO_CHAR(M.FEC_ENVIOMSJ,'DD/MM/YYYY') FEC_ENVIOMSJ, A.NU_ANN,A.NU_EMI,A.NU_DES,IDOSGD.PK_SGD_DESCRIPCION_DE_DOCUMENTO(B.CO_TIP_DOC_ADM)|| ' ' ||"
                + "(CASE B.TI_EMI ");
        ...
        sql.append("DE_TIP_DOC,UPPER(B.DE_ASU) DE_ASU,A.CO_LOC_DES CO_LOCAL,COALESCE( SUBSTR(IDOSGD.PK_SGD_DESCRIPCION_DE_LOCAL(A.CO_LOC_DES), 1, 100),NULL) DE_LOCAL, ");
        sql.append("B.CO_DEP_EMI CO_DEPENDENCIA,");
        sql.append("COALESCE( SUBSTR(IDOSGD.PK_SGD_DESCRIPCION_DE_DEPENDENCIA(B.CO_DEP_EMI), 1, 100),NULL) DE_DEPENDENCIA,");                    
        sql.append("(CASE A.TI_DES ");
        sql.append("WHEN '01' THEN IDOSGD.PK_SGD_DESCRIPCION_DE_DEPENDENCIA(A.CO_DEP_DES) ||'-'||IDOSGD.PK_SGD_DESCRIPCION_DE_NOM_EMP (A.CO_EMP_DES) ");
        ...
        sql.append("A.CO_MOT CO_TRAMITE,COALESCE( SUBSTR(IDOSGD.PK_SGD_DESCRIPCION_MOTIVO(A.CO_MOT), 1, 100),NULL) DE_TRAMITE, ");
        ...
        sql.append("(CASE A.TI_DES ");
        sql.append("WHEN '01' THEN TRIM(DEP.NODEP)||'/'||TRIM(DEP.NOPRV)||'/'||TRIM(DEP.NODIS) ");
        sql.append("WHEN '02' THEN TRIM(UBIDESTINO.NODEP)||'/'||TRIM(UBIDESTINO.NOPRV)||'/'||TRIM(UBIDESTINO.NODIS) ");
        sql.append("WHEN '03' THEN TRIM(UBIDESTINO.NODEP)||'/'||TRIM(UBIDESTINO.NOPRV)||'/'||TRIM(UBIDESTINO.NODIS) ");
        sql.append("WHEN '04' THEN TRIM(UBIDESTINO.NODEP)||'/'||TRIM(UBIDESTINO.NOPRV)||'/'||TRIM(UBIDESTINO.NODIS) ");
        sql.append("WHEN '05' THEN TRIM(DEP.NODEP)||'/'||TRIM(DEP.NOPRV)||'/'||TRIM(DEP.NODIS) ");
        sql.append("END ) DEPARTAMENTO,M.DE_TIP_MSJ,M.DE_TIP_ENV,");
        sql.append("(CASE WHEN M.DE_TIP_MSJ='COURRIER' THEN (SELECT TRIM(CPRO_RAZSOC) FROM IDOSGD.LG_PRO_PROVEEDOR WHERE CPRO_RUC=M.RE_ENV_MSJ) ELSE (SELECT TRIM(CEMP_APEPAT)||' '||TRIM(CEMP_APEMAT) ||' '|| TRIM(CEMP_DENOM) FROM IDOSGD.RHTM_PER_EMPLEADOS WHERE CEMP_NU_DNI=M.RE_ENV_MSJ) END) RE_ENV_MSJ, "); 
        sql.append("M.NU_SER_MSJ||'-'||M.AN_SER_MSJ NU_SERVICIO,");
        sql.append("COALESCE( TO_CHAR(A.FE_ENT_MSJ,'DD/MM/YYYY'),' ') FE_ENT_MSJ, ");
        sql.append("COALESCE( TO_CHAR(A.FE_DEV_MSJ,'DD/MM/YYYY'),' ') FE_DEV_MSJ,");
        sql.append("(SELECT DE_EST FROM IDOSGD.TDTR_ESTADOS WHERE CO_EST=A.ES_DOC_REC AND DE_TAB='TD_MENSAJERIA') EST_MSJ,A.ES_DOC_REC COD_EST_MSJ,M.DE_AMBITO,TO_CHAR(M.FE_PLA_MSJ,'DD/MM/YYYY') FE_PLA_MSJ,A.ES_PEN_MSJ,COALESCE( M.DE_OBS_MSJ,' ')||'  '||COALESCE( A.OB_MSJ,' ') OB_MSJ,COALESCE( A.MO_MSJ_DEV,' ') MO_MSJ_DEV, ");   
        sql.append("(M.FE_PLA_MSJ::date)-(M.FEC_ENVIOMSJ::date)+1 DIA_PLA,");
        sql.append("(CASE WHEN A.ES_DOC_REC='2' THEN IDOSGD.PK_SGD_MENSAJERIA_CAL_DIF_FECHAS_HABILES(M.FEC_ENVIOMSJ::date,current_date) ELSE ");
        sql.append(" IDOSGD.PK_SGD_MENSAJERIA_CAL_DIF_FECHAS_HABILES(M.FEC_ENVIOMSJ::date,A.FE_ENT_MSJ::date)  END) DIA_TRA,");
        sql.append("(CASE WHEN M.PE_ENV_MSJ='1' AND (IDOSGD.PK_SGD_MENSAJERIA_CAL_DIF_FECHAS_HABILES(M.FEC_ENVIOMSJ::date,M.FE_PLA_MSJ::date)-IDOSGD.PK_SGD_MENSAJERIA_CAL_DIF_FECHAS_HABILES(M.FEC_ENVIOMSJ::date,current_date))>0 AND A.ES_DOC_REC='2' ");
        sql.append("THEN (IDOSGD.PK_SGD_MENSAJERIA_CAL_DIF_FECHAS_HABILES(M.FEC_ENVIOMSJ::date,M.FE_PLA_MSJ::date))-(IDOSGD.PK_SGD_MENSAJERIA_CAL_DIF_FECHAS_HABILES(M.FEC_ENVIOMSJ::date,current_date)) ELSE ");
        sql.append("0 END) DIA_PEN, ");
        sql.append("(CASE WHEN M.PE_ENV_MSJ='1' AND (IDOSGD.PK_SGD_MENSAJERIA_CAL_DIF_FECHAS_HABILES(M.FEC_ENVIOMSJ::date,M.FE_PLA_MSJ::date)-IDOSGD.PK_SGD_MENSAJERIA_CAL_DIF_FECHAS_HABILES(M.FEC_ENVIOMSJ::date,current_date))<0 AND A.ES_DOC_REC='2' ");
        sql.append("THEN (IDOSGD.PK_SGD_MENSAJERIA_CAL_DIF_FECHAS_HABILES(M.FEC_ENVIOMSJ::date,current_date)-IDOSGD.PK_SGD_MENSAJERIA_CAL_DIF_FECHAS_HABILES(M.FEC_ENVIOMSJ::date,M.FE_PLA_MSJ::date)) ELSE CASE WHEN M.PE_ENV_MSJ='1' AND (IDOSGD.PK_SGD_MENSAJERIA_CAL_DIF_FECHAS_HABILES(M.FEC_ENVIOMSJ::date,M.FE_PLA_MSJ::date)-IDOSGD.PK_SGD_MENSAJERIA_CAL_DIF_FECHAS_HABILES(M.FEC_ENVIOMSJ::date,A.FE_ENT_MSJ::date))<0 AND A.ES_DOC_REC='3' ");
        sql.append("THEN (IDOSGD.PK_SGD_MENSAJERIA_CAL_DIF_FECHAS_HABILES(M.FEC_ENVIOMSJ::date,A.FE_ENT_MSJ::date)-IDOSGD.PK_SGD_MENSAJERIA_CAL_DIF_FECHAS_HABILES(M.FEC_ENVIOMSJ::date,M.FE_PLA_MSJ::date)) ELSE CASE WHEN M.PE_ENV_MSJ='1' AND  (IDOSGD.PK_SGD_MENSAJERIA_CAL_DIF_FECHAS_HABILES(M.FEC_ENVIOMSJ::date,M.FE_PLA_MSJ::date)-IDOSGD.PK_SGD_MENSAJERIA_CAL_DIF_FECHAS_HABILES(M.FEC_ENVIOMSJ::date,A.FE_DEV_MSJ::date))<0 AND A.ES_DOC_REC='4' THEN (IDOSGD.PK_SGD_MENSAJERIA_CAL_DIF_FECHAS_HABILES(M.FEC_ENVIOMSJ::date,A.FE_DEV_MSJ::date)-IDOSGD.PK_SGD_MENSAJERIA_CAL_DIF_FECHAS_HABILES(M.FEC_ENVIOMSJ::date,M.FE_PLA_MSJ::date)) ELSE 0 END END END) DIA_VEN, ");
        sql.append("(CASE WHEN A.ES_DOC_REC='3' AND M.PE_ENV_MSJ='1' AND IDOSGD.PK_SGD_MENSAJERIA_CAL_DIF_FECHAS_HABILES(M.FE_PLA_MSJ::date,A.FE_ENT_MSJ::date)>0 THEN IDOSGD.PK_SGD_MENSAJERIA_CAL_DIF_FECHAS_HABILES(M.FE_PLA_MSJ::date,A.FE_ENT_MSJ::date) ELSE 0 END) DIA_ENT,");
        sql.append("(CASE WHEN A.ES_DOC_REC='4' AND M.PE_ENV_MSJ='1' AND IDOSGD.PK_SGD_MENSAJERIA_CAL_DIF_FECHAS_HABILES(A.FE_PLA_DEV::date,A.FE_DEV_MSJ::date)>0 THEN IDOSGD.PK_SGD_MENSAJERIA_CAL_DIF_FECHAS_HABILES(A.FE_PLA_DEV::date,A.FE_DEV_MSJ::date) ELSE 0 END) DIA_DEV,");
        sql.append("B.ES_DOC_EMI,COALESCE( TO_CHAR(A.FE_PLA_DEV,'DD/MM/YYYY'),' ') FE_PLA_DEV, ");
        sql.append("COALESCE( TO_CHAR(B.FE_ENV_MES,'DD/MM/YYYY'),' ') FE_ENV_MES,COALESCE( TO_CHAR(B.FE_EMI,'DD/MM/YYYY'),' ') FE_EMI, ");
        sql.append("COALESCE( TO_CHAR(B.FEC_RECEPMP,'DD/MM/YYYY'),' ') FEC_RECEPMP, ");
        sql.append("COALESCE( A.nro_guia_devolucion,' ') as nro_guia_devolucion ");
        ...
        sql.append("LEFT JOIN IDOSGD.IDTUBIAS U ON U.UBDEP||U.UBPRV||U.UBDIS =L.CO_UBIGEO  ) DEP ON DEP.CO_DEPENDENCIA=A.co_dep_DES AND DEP.CCOD_LOCAL=a.co_loc_des " );
        sql.append(" LEFT JOIN IDOSGD.IDTUBIAS UBIDESTINO ON UBIDESTINO.UBDEP||UBIDESTINO.UBPRV||UBIDESTINO.UBDIS =a.CCOD_DPTO||a.CCOD_PROV||a.CCOD_DIST  ");
        sql.append(" INNER JOIN IDOSGD.TD_MENSAJERIA M ON A.NU_MSJ=M.NU_MSJ "); 
...
//        System.out.println("EMPLEADO"+buscarDocumentoCargaMsjBean.getCoEmpleado());
        ...
        if (buscarDocumentoCargaMsjBean.getEsFiltroFecha() != null
                ...
                if (vFeEmiIni != null && vFeEmiIni.trim().length() > 0
                    ...
                    sql.append(" AND M.FEC_ENVIOMSJ between TO_DATE(:pFeEmiIni,'dd/mm/yyyy') AND TO_DATE(:pFeEmiFin,'dd/mm/yyyy') + TIME '23:59:59' ");
...
//            System.out.println(buscarDocumentoCargaMsjBean.getBusNuMsj());
            ...
            if (buscarDocumentoCargaMsjBean.getBusDesti()!= null && buscarDocumentoCargaMsjBean.getBusDesti().trim().length() > 0  ) {
                ...
                sql.append(" AND UPPER((CASE A.TI_DES ");
                sql.append("WHEN '01' THEN IDOSGD.PK_SGD_DESCRIPCION_DE_DEPENDENCIA(A.CO_DEP_DES) ||'-'||IDOSGD.PK_SGD_DESCRIPCION_DE_NOM_EMP (A.CO_EMP_DES) ");
                sql.append("WHEN '02' THEN IDOSGD.PK_SGD_DESCRIPCION_DE_PROVEEDOR (A.NU_RUC_DES) ");
                sql.append("WHEN '03' THEN IDOSGD.PK_SGD_DESCRIPCION_ANI_SIMIL (A.NU_DNI_DES) ");
                sql.append("WHEN '04' THEN IDOSGD.PK_SGD_DESCRIPCION_OTRO_ORIGEN (A.CO_OTR_ORI_DES) ");
                sql.append("WHEN '05' THEN IDOSGD.PK_SGD_DESCRIPCION_DE_DEPENDENCIA(A.CO_DEP_DES) ||'-'||IDOSGD.PK_SGD_DESCRIPCION_DE_NOM_EMP (A.CO_EMP_DES)");
                sql.append("END )) LIKE '%'||UPPER(:pDesti)||'%' ");
                        ...
                        {    
                            ...
                            if (palabra.trim().length()>0)
                            {
                                if (cont==0)
                                {
                                    ...
                                    sql.append("WHEN '01' THEN B.NU_DOC_EMI || '-' || B.NU_ANN || '/' || B.DE_DOC_SIG ");
                                    sql.append("WHEN '05' THEN B.NU_DOC_EMI || '-' || B.NU_ANN || '/' || B.DE_DOC_SIG ");
                                    sql.append("ELSE B.DE_DOC_SIG END )) LIKE '%'||UPPER(:pNuDoc)||'%' ");
    ...
    public String updMensajeriaDocumento(DescargaMensajeBean descargaMensaje) {
        ...
        if (descargaMensaje.getEs_pen_msj().equals("0"))
        {
            ...
            descargaMensaje.setEs_pen_dev("");
                ...
                this.spDescargamsj = new SimpleJdbcCall(this.dataSource)
                        .withSchemaName("idosgd")
                        .withFunctionName("pk_sgd_mensajeria_descargamsj") 
    ...
    public String insArchivoAnexoDes(final DocumentoAnexoDesBean docAnexo,final  InputStream archivoAnexo,final int size) {
       ...
       /* System.out.println("eeeeee");
        ...
        if (pcount.equals("0"))
        {
            ...
            + "nu_des, \n"
            + "bl_doc )\n"
            + "values(?,?,CAST(? AS smallint),REGEXP_REPLACE(?, '[^-_ABCDEFGHIJKLMNÑOPQRSTUVWXYZabcdefghijklmnñopqrstuvwxyz0-9ÁÉÍÓÚáéíóú. ]', ''),REGEXP_REPLACE(?, '[^-_ABCDEFGHIJKLMNÑOPQRSTUVWXYZabcdefghijklmnñopqrstuvwxyz0-9ÁÉÍÓÚáéíóú. ]', ' '),?,now(),?,now(),to_char(now(),'yyyymmdd'),CAST(? AS int),?)");
        ...
        else
        {
            sql.append("update idosgd.tdtv_anexos set\n"  
                ...
                + "bl_doc=?\n"
                + "where \n"
                + "nu_ann=? and nu_emi=? and nu_des=CAST(? AS int)");
    ...
    public String insArchivoActa(final MensajesConsulBean docAnexo,final  InputStream archivoAnexo,final int size) {
       ...
       /* System.out.println("eeeeee");
        ...
        if (pcount.equals("0"))
        {
            ...
            + "fe_use_cre,\n"
            + "bl_doc )\n"
            + "values(?,?,cast(? as numeric),?,REGEXP_REPLACE(?, '[^-_ABCDEFGHIJKLMNÑOPQRSTUVWXYZabcdefghijklmnñopqrstuvwxyz0-9ÁÉÍÓÚáéíóú. ]', ''),?,now(),?)");
        ...
        else
        {
            sql.append("update idosgd.TD_DET_MENSAJERIA set\n"  
                ...
                + "bl_doc=?\n"
                + "where \n"
                + "nu_ann=? and nu_emi=? and nu_des=cast(? as numeric) and nu_acta=?");
    ...
    public String getUltimoAnexo(String pnuAnn, String pnuEmi) {
        ...
        StringBuffer sql = new StringBuffer();
        sql.append("select (COALESCE(max(nu_ane),0) + 1)::text nu_ane \n"
                + "from idosgd.tdtv_anexos \n"
                + "where nu_ann = ? \n"
                + "and nu_emi = ? ");
    ...
    public String delMensajeriaDocumento(DescargaMensajeBean descargaMensaje) {
       ...
       /* StringBuffer sql = new StringBuffer(); 
        ...
        this.spDescargamsj = new SimpleJdbcCall(this.dataSource)
                .withSchemaName("idosgd")
                .withFunctionName("pk_sgd_mensajeria_delmsj")  
        .withoutProcedureColumnMetaDataAccess()
...
//        List<MensajesConsulBean> list = new ArrayList<MensajesConsulBean>();
        ...
        sql.append("SELECT  ");
        sql.append("A.NU_MSJ ::text ,TO_CHAR(M.FEC_ENVIOMSJ,'DD/MM/YYYY') FEC_ENVIOMSJ, A.NU_ANN,A.NU_EMI,A.NU_DES::text,idosgd.PK_SGD_DESCRIPCION_DE_DOCUMENTO(B.CO_TIP_DOC_ADM)|| ' ' ||"
                + "( CASE B.TI_EMI ");
        sql.append(" WHEN '01' THEN B.NU_DOC_EMI || '-' || B.NU_ANN || '/' || B.DE_DOC_SIG ");
        sql.append(" WHEN '05' THEN B.NU_DOC_EMI || '-' || B.NU_ANN || '/' || B.DE_DOC_SIG ");
        sql.append(" ELSE B.DE_DOC_SIG END)");
        sql.append("DE_TIP_DOC,UPPER(B.DE_ASU) DE_ASU,A.CO_LOC_DES CO_LOCAL,COALESCE(A.CO_LOC_DES,SUBSTR(idosgd.PK_SGD_DESCRIPCION_DE_LOCAL(A.CO_LOC_DES), 1, 100),NULL) DE_LOCAL, ");
        sql.append("B.CO_DEP_EMI CO_DEPENDENCIA,");
        sql.append("COALESCE(B.CO_DEP_EMI,SUBSTR(idosgd.PK_SGD_DESCRIPCION_DE_DEPENDENCIA(B.CO_DEP_EMI), 1, 100),NULL) DE_DEPENDENCIA,");                    
        sql.append("(CASE A.TI_DES ");
        sql.append(" WHEN '01' THEN idosgd.PK_SGD_DESCRIPCION_DE_DEPENDENCIA(A.CO_DEP_DES) ||'-'||idosgd.PK_SGD_DESCRIPCION_DE_NOM_EMP (A.CO_EMP_DES) ");
        sql.append(" WHEN '02' THEN idosgd.PK_SGD_DESCRIPCION_DE_PROVEEDOR (A.NU_RUC_DES) ");
        sql.append(" WHEN '03' THEN idosgd.PK_SGD_DESCRIPCION_ANI_SIMIL (A.NU_DNI_DES) ");
        ...
        sql.append("A.TI_DES CO_TIPO_DESTINO,");
        sql.append(" (CASE A.TI_DES ");
        sql.append(" WHEN '01' THEN TRIM(DEP.DE_DIRECCION_LOCAL) ");
        ...
        sql.append(" WHEN '05' THEN TRIM(DEP.DE_DIRECCION_LOCAL) END ) DIRECCION,");
        sql.append(" (CASE A.TI_DES ");
        sql.append(" WHEN '01' THEN TRIM(DEP.NODEP)||'/'||TRIM(DEP.NOPRV)||'/'||TRIM(DEP.NODIS) ");
        ...
        sql.append(" WHEN '05' THEN TRIM(DEP.NODEP)||'/'||TRIM(DEP.NOPRV)||'/'||TRIM(DEP.NODIS) END ) DEPARTAMENTO,M.DE_TIP_MSJ,M.DE_TIP_ENV,");
        sql.append("(CASE WHEN M.DE_TIP_MSJ='COURRIER' THEN (SELECT TRIM(CPRO_RAZSOC) FROM idosgd.LG_PRO_PROVEEDOR WHERE CPRO_RUC=M.RE_ENV_MSJ) ELSE (SELECT TRIM(CEMP_APEPAT)||' '||TRIM(CEMP_APEMAT) ||' '|| TRIM(CEMP_DENOM) FROM idosgd.RHTM_PER_EMPLEADOS WHERE CEMP_NU_DNI=M.RE_ENV_MSJ) END) RE_ENV_MSJ, ");
        sql.append("M.NU_SER_MSJ||'-'||M.AN_SER_MSJ NU_SERVICIO,");
        sql.append("COALESCE( TO_CHAR(A.FE_ENT_MSJ,'DD/MM/YYYY'),' ') FE_ENT_MSJ, ");
        sql.append("COALESCE( TO_CHAR(A.FE_DEV_MSJ,'DD/MM/YYYY'),' ') FE_DEV_MSJ,");
        sql.append("(SELECT DE_EST FROM idosgd.TDTR_ESTADOS WHERE CO_EST=A.ES_DOC_REC AND DE_TAB='TD_MENSAJERIA') EST_MSJ,A.ES_DOC_REC COD_EST_MSJ,M.DE_AMBITO,M.FE_PLA_MSJ,A.ES_PEN_MSJ,COALESCE(A.OB_MSJ,A.OB_MSJ,' ') OB_MSJ,COALESCE(A.MO_MSJ_DEV,A.MO_MSJ_DEV,' ') MO_MSJ_DEV, "); 
        sql.append("COALESCE( TO_CHAR(A.HO_ENT_MSJ,'HH24:MI'),' ') HO_ENT_MSJ,");
        sql.append("COALESCE( TO_CHAR(A.HO_DEV_MSJ,'HH24:MI'),' ') HO_DEV_MSJ,");
        ...
        sql.append(" A.nro_guia_devolucion, B.NU_COR_EMI::text||'-'|| A.NU_DES::text||'-1' as nu_Acta_Vis1, B.NU_COR_EMI::text||'-'|| A.NU_DES::text||'-2' as nu_Acta_Vis2, ");
        sql.append(" DM1.FEC_NOT_ACTA as fe_Acta_Vis1,COALESCE(DM1.ES_NOT_ACTA,'2') as es_Acta1_msj, ");
        sql.append(" DM2.FEC_NOT_ACTA as fe_Acta_Vis2,COALESCE(DM2.ES_NOT_ACTA,'2') as es_Acta2_msj, ");
        sql.append(" COALESCE(DM1.DE_RUT_ORI,'') as archivo_Acta1, ");
        sql.append(" COALESCE(DM2.DE_RUT_ORI,'') as archivo_Acta2, ");
        sql.append(" COALESCE(TO_CHAR(DM1.FEC_NOT_ACTA,'DD/MM/YYYY'),' ') as fe_Acta_Vis1,COALESCE(DM1.ES_NOT_ACTA,'2') as es_Acta1_msj, ");
        sql.append("  COALESCE(TO_CHAR(DM2.FEC_NOT_ACTA,'DD/MM/YYYY'),' ') as fe_Acta_Vis2,COALESCE(DM2.ES_NOT_ACTA,'2') as es_Acta2_msj, ");
        sql.append(" COALESCE(DM1.DE_RUT_ORI,' ') as archivo_Acta1, ");
        sql.append(" COALESCE(DM2.DE_RUT_ORI,' ') as archivo_Acta2, ");
        sql.append(" COALESCE( CARGO.NU_ANE::text,' ') as archivo_Cargo ");
        sql.append(" FROM idosgd.TDTV_DESTINOS A INNER JOIN idosgd.TDTV_REMITOS B ON A.NU_ANN = B.NU_ANN AND A.NU_EMI = B.NU_EMI ");
        sql.append(" LEFT JOIN ( SELECT D.CO_DEPENDENCIA,D.DE_DEPENDENCIA,D.DE_CORTA_DEPEN,L.DE_NOMBRE_LOCAL,L.DE_DIRECCION_LOCAL,NODEP,NOPRV,NODIS ,L.CCOD_LOCAL ");
        sql.append(" FROM idosgd.RHTM_DEPENDENCIA D ");
        sql.append(" INNER JOIN idosgd.SITM_LOCAL_DEPENDENCIA DL ON DL.CO_DEP=D.CO_DEPENDENCIA ");
        ...
        sql.append(" LEFT JOIN idosgd.IDTUBIAS U ON U.UBDEP||U.UBPRV||U.UBDIS =L.CO_UBIGEO) DEP ON DEP.CO_DEPENDENCIA=A.CO_DEP_DES AND DEP.CCOD_LOCAL=A.CO_LOC_DES ");
        sql.append(" INNER JOIN idosgd.TD_MENSAJERIA M ON A.NU_MSJ=M.NU_MSJ "); 
        sql.append(" LEFT JOIN ( select  nu_ann,nu_emi,nu_des, COUNT(1) TOTAL    from idosgd.tdtv_anexos  GROUP BY nu_ann,nu_emi,nu_des ) TB ON  TB.nu_ann = A.nu_ann  AND TB.nu_emi=A.nu_emi AND TB.nu_des=A.nu_des "); 
        sql.append(" LEFT JOIN idosgd.TD_DET_MENSAJERIA DM1 ON DM1.NU_ANN=A.NU_ANN AND DM1.NU_EMI=A.NU_EMI AND DM1.NU_DES=A.NU_DES AND DM1.NU_ACTA= B.NU_COR_EMI::text||'-'|| A.NU_DES::text||'-1' ");
        sql.append(" LEFT JOIN idosgd.TD_DET_MENSAJERIA DM2 ON DM2.NU_ANN=A.NU_ANN AND DM2.NU_EMI=A.NU_EMI AND DM2.NU_DES=A.NU_DES AND DM2.NU_ACTA= B.NU_COR_EMI::text||'-'|| A.NU_DES::text||'-2' ");
        sql.append(" LEFT JOIN idosgd.TDTV_ANEXOS CARGO ON CARGO.NU_ANN=A.NU_ANN AND CARGO.NU_EMI=A.NU_EMI AND CARGO.NU_DES=A.NU_DES  ");//AND CARGO.DE_DET='CARGO_NOTIFICACION'||TO_CHAR(A.NU_DES)||'.pdf'
        sql.append(" WHERE  A.NU_ANN =? AND A.NU_EMI =? AND A.NU_DES =CAST(? AS BIGINT) AND  A.NU_MSJ =CAST(? AS BIGINT) ");
            // Parametros Basicos       
...
//        System.out.println(sql.toString());
    ...
    public DocumentoObjBean getNombreArchivoMsj(String pnuAnn, String pnuEmi, String pnuAnexo) {
        ...
        sql.append("select \n" +
                    ...
                    "COALESCE(octet_length(bl_doc),0) nu_Tamano\n" +
                    "from idosgd.tdtv_anexos\n" +
                    "where nu_ann = ?\n" +
                    "and nu_emi = ?\n" +
                    "and nu_des = ?");
    ...
    public String revMensajeriaDocumento(DescargaMensajeBean descargaMensaje) {
        ...
        this.spDescargamsj = new SimpleJdbcCall(this.dataSource)
                .withSchemaName("idosgd")
                 .withFunctionName("pk_sgd_mensajeria_revmsj")  
        .withoutProcedureColumnMetaDataAccess()
        .useInParameterNames("pnu_ann", "pnu_emi", "pnu_des", "pnu_msj")
                //descargamsj(pnu_ann tdtv_remitos.nu_ann%TYPE,pnu_emi tdtv_remitos.nu_emi%TYPE,pnu_des tdtv_destinos.nu_des%TYPE,pnu_msj tdtv_destinos.nu_msj%TYPE,pfecha tdtv_destinos.fec_finmsj%TYPE,pob_msj tdtv_destinos.ob_msj%TYPE,pmo_msj_dev tdtv_destinos.mo_msj_dev%TYPE,pes_pen_msj tdtv_destinos.es_pen_msj%TYPE,pest VARCHAR2);
        .declareParameters(
    ...
    public List<DependenciaBean> getLsOficina() {
        ...
        sql.append("SELECT deDependencia,coDependencia FROM (SELECT COALESCE(SUBSTR(idosgd.PK_SGD_DESCRIPCION_DE_DEPENDENCIA(CO_DEP_EMI), 1, 100),NULL) deDependencia,CO_DEP_EMI coDependencia \n" +
                    "FROM idosgd.TDTV_REMITOS \n" +
                    "WHERE CO_DEP_EMI IS NOT NULL \n" +
                    "UNION SELECT '.: TODOS :.',NULL  ) TB \n" +
    ...
    public String selectCalFechaPlazo(DescargaMensajeBean descargaMensaje) {
        ...
        try{
            ...
            sqlQry2.append("select idosgd.PK_SGD_CONSULTAS_CALCULAR_DIAS_HABILES(TO_DATE('"+descargaMensaje.getFe_ent_msj()+"','DD/MM/YYYY'),1) ");  
            ...
            sqlQry.append("select  idosgd.PK_SGD_CONSULTAS_CALCULAR_DIAS_HABILES(TO_DATE('"+snufechai+"','DD/MM/YYYY'),"+diadev+") ");  

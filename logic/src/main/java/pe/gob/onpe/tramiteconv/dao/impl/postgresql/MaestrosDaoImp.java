...
public class MaestrosDaoImp extends SimpleJdbcDaoBase implements MaestrosDao{
    ...
    @Override
        ...
        if(ptiAcceso!=null&&(ptiAcceso.equals("0")||ptiAcceso.equals("2"))){
            ...
            sql.append("		   co_dep_REF = ?/*:global.vcod_dep*/");
    ...
    @Override
        ...
        if(annio.length() == 1 || annio == "0"){
             ...
             sql.append(" a.nu_ann = ?/*:b_01_ann.de_anio*/");
        }
        ...
        try {
            ...
            usuarioConfigBean.getTiConsulta(),usuarioConfigBean.getTiConsulta(),usuarioConfigBean.getCempCodemp(),usuarioConfigBean.getCoDep(),usuarioConfigBean.getTiConsulta(),usuarioConfigBean.getTiConsulta(),usuarioConfigBean.getCempCodemp()});
            } else {
...
//            }
    ...
    public List<EmpleadoBean> listEmpleadoElaboradoPorMP(String pcodDependencia,String ptiAcceso,String pcoEmpleado){
        ...
        boolean bquery=false;
            ...
            sql.append(" SELECT DISTINCT DE_EMP_REC NOMBRE,CO_EMP_RES CEMP_CODEMP ");
            sql.append(" FROM (SELECT ");
            sql.append(" IDOSGD.PK_SGD_DESCRIPCION_DE_NOM_EMP(R.CO_EMP_RES) DE_EMP_REC, R.CO_EMP_RES ");
            ...
            sql.append(" INNER JOIN (SELECT Y.NU_ANN, Y.NU_EMI, Y.FE_FINALIZA,IDOSGD.PK_SGD_DESCRIPCION_FU_DIA_TRA (CAST(now() as date),Y.FE_FINALIZA) NU_DIA_FAL FROM (SELECT X.NU_ANN, X.NU_EMI, ");
            sql.append(" CAST (X.FE_EMI as date)+ IDOSGD.PK_SGD_DESCRIPCION_FU_DIA_MAS (CAST (X.FE_EMI as date),NU_DIA_ATE) AS FE_FINALIZA ");
            sql.append(" FROM IDOSGD.TDTV_REMITOS X) Y) E ON E.NU_ANN=R.NU_ANN AND E.NU_EMI=R.NU_EMI");

...
public class ConsultaEmiDocExtDaoImp extends SimpleJdbcDaoBase implements ConsultaEmiDocExtDao{
    ...
    public List<DocumentoExtConsulBean> getDocumentosExternos(BuscarDocumentoExtConsulBean buscarDocExt) {
        ...
        sql.append(" A.CO_DEP");
        sql.append(" FROM IDOSGD.TDTV_REMITOS A WITH (NOLOCK),IDOSGD.TDTX_REMITOS_RESUMEN B WITH (NOLOCK),IDOSGD.TDTC_EXPEDIENTE C WITH (NOLOCK) ");
         if (buscarDocExt.getBusCoGrupoExt()!= null && buscarDocExt.getBusCoGrupoExt().trim().length() > 0) {
             sql.append(",TDTD_DEP_GRU_VAR G WITH (NOLOCK) ");
         }
...
//            }
    ...
    public DocumentoExtConsulBean getDocumentoExtConsulBean(String nuAnn, String nuEmi){
        ...
        sql.append("SELECT A.NU_ANN,A.NU_EMI,A.NU_COR_EMI,B.NU_EXPEDIENTE,\n" +
                ...
                "FROM IDOSGD.TDTV_REMITOS A  WITH (NOLOCK) LEFT JOIN IDOSGD.TDTC_EXPEDIENTE B WITH (NOLOCK) \n" +
...
//            }
    ...
    public List<DocumentoExtConsulBean> getListaReporteBusqueda(BuscarDocumentoExtConsulBean buscarDocumentoExtConsulBean) {
        ...
        prutaReporte.append("	     FROM IDOSGD.TDTR_PROCESOS_EXP WITH (NOLOCK)  ");
        ...
        prutaReporte.append("     IDOSGD.TDTC_EXPEDIENTE C  WITH (NOLOCK) ");
           //YUAL

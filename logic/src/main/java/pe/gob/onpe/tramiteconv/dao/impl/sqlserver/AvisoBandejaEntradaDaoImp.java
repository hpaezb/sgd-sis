...
"                    FROM (                    \n" +
"                    SELECT count(1) Nu_Can FROM IDOSGD.Tdtv_Remitos r  WITH (NOLOCK) \n" +
"                    WHERE r.Es_Doc_Emi IN ('7') \n" +
"                    AND r.Ti_Emi = '01'                 \n" +
"                    AND r.Co_Dep_Emi = :pCoDep \n" +
"                    GROUP BY r.Co_Dep_Emi \n" +
...
"                       INNER  JOIN IDOSGD.TDTV_PERSONAL_VB VB  WITH (NOLOCK) ON R.NU_ANN=VB.NU_ANN AND R.NU_EMI=VB.NU_EMI\n" +
...
"                        isnull([1],0)+isnull([2],0)+isnull([3],0)  AS Nu_Can,:pCoDep co_dep , 'O' co_bandeja,\n" +
"                        isnull([1],0) nuCanNormal,isnull([2],0) nuCanUrgente ,isnull([3],0) nuCanMuyUrgente \n" +
"                        FROM (\n" +
"                        SELECT  t.Co_Pri,COUNT(1) Nu_Can \n" +
"                                                  FROM IDOSGD.Tdtv_Remitos  r WITH (NOLOCK) \n" +
"                                                  INNER JOIN IDOSGD.Tdtv_Destinos t WITH (NOLOCK) ON r.Nu_Ann = t.Nu_Ann \n" +
"                                                   AND r.Nu_Emi = t.Nu_Emi \n" +
...
"                                               --AND t.Co_Pri = '3'\n" +
"                                                   AND t.Co_Dep_Des = :pCoDep\n" +
"                                                   AND t.CO_EMP_DES in ( SELECT CO_EMPLEADO FROM IDOSGD.RHTM_DEPENDENCIA WITH (NOLOCK) WHERE CO_DEPENDENCIA= :pCoDep)   \n" +
"                                                  LEFT JOIN ( SELECT COUNT(1) totalproceso_ref,ref.nu_ann_ref,ref.nu_emi_ref,ref.nu_des_ref\n" +
"                                                  FROM IDOSGD.tdtr_referencia ref WITH (NOLOCK) \n" +
"                                                  INNER JOIN IDOSGD.tdtv_remitos b WITH (NOLOCK) ON ref.nu_ann = b.nu_ann and ref.nu_emi = b.nu_emi\n" +
...
"                                                  where isnull(totalproceso_ref,0)=0\n" +
"                                                  GROUP BY t.Co_Pri" +
"                           union select '',0 \n" +
"                        )TB \n" +
"                         PIVOT (AVG(Nu_Can) FOR Co_Pri IN ([1],[2],[3])) as tbp ");
        }
        else if (bandeja.equals("BANDEJADELEGADOS")) {  
         sql.append("SELECT 5 orden,'P' Ti_Pen, 'DELEGADOS' De_Pen, 'Documentos recibidos' de_resumen, isnull([1],0)+isnull([2],0)+isnull([3],0)  AS Nu_Can \n" +
"                    ,:pCoDep co_dep , 'P' co_bandeja,isnull([1],0) nuCanNormal,isnull([2],0) nuCanUrgente ,isnull([3],0) nuCanMuyUrgente FROM (\n" +
"                    SELECT  t.Co_Pri , COUNT(1) Nu_Can \n" +
"                          FROM IDOSGD.Tdtv_Remitos  r  WITH (NOLOCK)\n" +
"                           INNER JOIN    IDOSGD.Tdtv_Destinos t WITH (NOLOCK) ON r.Nu_Ann = t.Nu_Ann \n" +
"                           AND r.Nu_Emi = t.Nu_Emi \n" +
...
"                          --AND t.Co_Pri = '3'\n" +
"                           AND t.Co_Dep_Des = :pCoDep\n" +
"                           AND NOT t.CO_EMP_DES in ( SELECT CO_EMPLEADO FROM IDOSGD.RHTM_DEPENDENCIA WITH (NOLOCK) WHERE CO_DEPENDENCIA= :pCoDep)   \n" +
"                          LEFT JOIN ( SELECT COUNT(1) totalproceso_ref,ref.nu_ann_ref,ref.nu_emi_ref,ref.nu_des_ref\n" +
"                          FROM IDOSGD.tdtr_referencia ref WITH (NOLOCK) \n" +
"                          INNER JOIN IDOSGD.tdtv_remitos b  WITH (NOLOCK)ON ref.nu_ann = b.nu_ann and ref.nu_emi = b.nu_emi\n" +
"                          WHERE  b.es_eli = '0'  and b.es_doc_emi <> '9'  \n" +
"                          GROUP BY ref.nu_ann_ref,ref.nu_emi_ref,ref.nu_des_ref ) TB  ON   TB.nu_ann_ref=r.nu_ann AND TB.nu_emi_ref=r.NU_EMI AND TB.nu_des_ref=t.nu_des    \n" +
"                          where isnull(totalproceso_ref,0)=0\n" +
"                          group by t.Co_Pri  union select '',0 \n" +
"                    )TB \n" +
"                    PIVOT (AVG(Nu_Can) FOR Co_Pri IN ([1],[2],[3])) tbp ");
        }
...
"                         AND r.Ti_Emi = '01'                 \n" +
"                            AND r.Co_Dep_Emi = :pCoDep\n" +
"                            AND convert(nvarchar,r.FE_EMI,103)=convert(nvarchar,getdate(),103) \n" +
"                          GROUP BY r.Co_Dep_Emi \n" +
"                          union select 0 nu_can  \n" +
"                          ) tb ");
        }
...
"                          AND r.CO_TIP_DOC_ADM in ( SELECT CDOC_TIPDOC FROM IDOSGD.SI_MAE_TIPO_DOC WITH (NOLOCK) WHERE IN_DOC_SALIDA=1 ) AND TI_EMI='01' \n" +
"                          AND r.Es_Eli = '0'\n" +
"                          AND r.Co_Dep_Emi = :pCoDep\n" +
"                       GROUP BY r.Co_Dep_Emi");
        }
...
"                           COUNT(1) Nu_Can, :pCoDep co_dep \n" +
"                      FROM IDOSGD.Tdtv_Remitos  r WITH (NOLOCK) \n" +
"                       INNER JOIN IDOSGD.Tdtv_Destinos t  WITH (NOLOCK) ON r.Nu_Ann = t.Nu_Ann AND r.Nu_Emi = t.Nu_Emi\n" +

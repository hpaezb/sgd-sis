...
public class DocumentoObjDaoImp extends SimpleJdbcDaoBase implements DocumentoObjDao {
    ...
    public DocumentoDatoBean getDatosDoc(String pnuAnn, String pnuEmi){
        ...
        sql.append("select \n" +
                    ...
                    "co_dep_emi,\n" +
                    "co_emp_emi,\n" +
                    "CAST(nu_cor_emi AS text) nu_cor_emi, \n" +
                    "ti_emi,\n" +
                    "ti_cap,\n" +
                    "es_doc_emi,\n" +
                    ...
                    "to_char(CURRENT_TIMESTAMP,'DD/MM/YYYY') fecha_actual,\n" +
                    "to_char(CURRENT_TIMESTAMP,'YYYY-MM-DD') fecha_firma,\n" +
                    "INITCAP(IDOSGD.PK_SGD_DESCRIPCION_DE_DISTRITO_LOCAL(co_loc_emi)) de_Lugar,COALESCE(COD_VER_EXT,' ') as coVerExt \n" +
    ...
    public  DocumentoObjBean getNombreArchivo(String pnuAnn, String pnuEmi, String ptiCap){
        ...
        sql.append("select \n" +
                    ...
                    "COALESCE(octet_length(w_bl_doc),0) w_Nu_Tamano \n"+                
                    "from IDOSGD.tdtv_archivo_doc \n" +
                    "where nu_ann = ?\n" +
                    "and nu_emi = ?");
    ...
    public DocumentoDatoBean CargarCabeceraReporte(String pnuAnn, String pnuEmi) {        
        ...
        sql.append("SELECT	IDOSGD.PK_SGD_DESCRIPCION_DE_DEPENDENCIA(a.co_dep_emi) de_dep_emi,\n" +
                    ...
                    "	CAST(COALESCE(NU_DOC, 0) AS VARCHAR) NU_DOC,\n" +
                    "	IDOSGD.PK_SGD_DESCRIPCION_DE_NOM_EMP(a.co_emp_emi) de_emp_emi,\n" +
                    "	IDOSGD.PK_SGD_DESCRIPCION_DE_CARGO_DOCUMENTO(a.co_dep_emi,a.co_emp_emi) cargo_emp_emi,\n" +
    ...
    public List<DocumentoDatoBean> CargarSubReporte1(String pnuAnn, String pnuEmi) {
        StringBuilder sql = new StringBuilder();
            sql.append("select    a.nu_ann NU_ANN_R,\n" +
                        ...
                        "     INNER JOIN IDOSGD.TDTV_REMITOS b ON a.nu_emi_ref = b.nu_emi and a.nu_ann_ref = b.nu_ann \n" +
                        "where  a.nu_ann= ?\n" +
                        "and a.nu_emi= ?");
...
//                        "		ELSE IDOSGD.PK_SGD_DESCRIPCION_DE_DEPENDENCIA(b.co_dep_des) || '\n' || (\n" +
                        "		IDOSGD.PK_SGD_DESCRIPCION_DE_DEPENDENCIA(b.co_dep_des) || '\n' || (\n" +
                        "				CASE b.ti_des\n" +
                        "					WHEN '01' THEN IDOSGD.PK_SGD_DESCRIPCION_DE_NOM_EMP(b.co_emp_des)\n" +
                        "					WHEN '02' THEN IDOSGD.PK_SGD_DESCRIPCION_DE_PROVEEDOR(B.NU_RUC_DES)\n" +
                        "					WHEN '03' THEN IDOSGD.PK_SGD_DESCRIPCION_ANI_SIMIL(b.Nu_Dni_Des)\n" +
                       ...
                       // "	END DE_DEP_EMP,\n" +
                        ...
                        "	IDOSGD.PK_SGD_DESCRIPCION_DE_NU_EXPEDIENTE(A.NU_ANN_EXP,A.NU_SEC_EXP) NU_EXPEDIENTE\n" +
                        "FROM IDOSGD.tdtv_remitos a \n" +
                        "     INNER JOIN IDOSGD.tdtv_destinos b ON  a.nu_ann = b.nu_ann AND a.nu_emi = b.nu_emi \n" +
                        "WHERE a.es_eli = '0'\n" +
                        "AND b.es_eli = '0'\n" +                        
                        "AND a.nu_ann = ?\n" +
                        "AND a.nu_emi = ?\n" +
    ...
    public List<DocumentoNotificacionBean> CargarReporteNotificacion(String pnuAnn, String pnuEmi, String tiOpe) {
         StringBuffer sql = new StringBuffer();
            sql.append("SELECT  'N° '|| a.nu_cor_emi ||'-'|| b.nu_des ||'-'||'"+tiOpe+"' numero_id, \n" +
                        ...
                        "    b.CDIR_REMITE \n" +
                        "    || ',' || IDOSGD.PK_SGD_DESCRIPCION_fu_get_departamento(b.ccod_dpto) \n" +
                        "    || '-' || IDOSGD.PK_SGD_DESCRIPCION_fu_get_provincia (b.ccod_dpto, b.ccod_prov) \n" +
                        "    || '-' ||  IDOSGD.PK_SGD_DESCRIPCION_fu_get_distrito (b.ccod_dpto,b.ccod_prov,b.ccod_dist) direccion, '' referencia,'' folio,\n" +
                        "    (SELECT DE_PAR FROM IDOSGD.TDTR_PARAMETROS WHERE CO_PAR='NOMBRE_ANIO'   LIMIT 1) nombre_anio_1,\n" +
                        "    (SELECT DE_PAR FROM IDOSGD.TDTR_PARAMETROS WHERE CO_PAR='NOMBRE_ANIO2'   LIMIT 1) nombre_anio_2\n" +
                        "FROM IDOSGD.tdtv_remitos a \n" +
                        "inner join IDOSGD.tdtv_destinos b on a.nu_ann = b.nu_ann and  a.nu_emi = b.nu_emi\n" +
...
//                         blobStream = documento.getBinaryStream();
                        blobStream = new ByteArrayInputStream(fileBytes);
    ...
    public String getInFirmaDoc(String pcoDep,String pcoTipoDoc){
        StringBuffer sql = new StringBuffer();
        String result = null;
        sql.append(" SELECT CASE WHEN es_obl_firma='0' THEN 'N' ELSE 'F' END \n" +
    ...
    public DocumentoObjBlock getDatosDocBlock(String pnuAnn,String pnuEmi){
        ...
        sql.append("SELECT RB.NU_ANN,RB.NU_EMI,RB.NU_SEC_FIRMA,\n" +
                    "RB.FECHA_FIRMA,RB.TIPO_FIRMA,CURRENT_TIMESTAMP FECHA_ACTUAL\n" +

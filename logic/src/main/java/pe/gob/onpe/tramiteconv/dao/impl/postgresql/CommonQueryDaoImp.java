...
public class CommonQueryDaoImp extends SimpleJdbcDaoBase implements CommonQueryDao {
    ...
    public List<DestinatarioOtroOrigenBean> getLstOtrosOrigenes(String pdescripcion){
        ...
        sql.append("SELECT DE_APE_PAT_OTR||' '|| DE_APE_MAT_OTR||', '|| DE_NOM_OTR || ' - ' || DE_RAZ_SOC_OTR DESCRIPCION,\n"
                ...
                + "COALESCE(UB_DEP,'') ID_DEPARTAMENTO, COALESCE(UB_PRO,'') ID_PROVINCIA, COALESCE(UB_DIS,'') ID_DISTRITO, COALESCE(DE_DIR_OTRO_ORI,'') DE_DIRECCION, COALESCE(DE_EMAIL,'') DE_CORREO,COALESCE(DE_TELEFO,'') TELEFONO \n"
                ...
                + ") and COALESCE(CO_TIP_OTR_ORI,'--') not in('09')\n" 
                 + " order by 1 desc");
    ...
    public List<CiudadanoBean> getLstCiudadanos(String sDescCiudadano){
        ...
        sql.append("select I.NULEM NU_DOCUMENTO,substring(RTRIM(I.DEAPP)+' '+RTRIM(I.DEAPM)+' '+RTRIM(I.DENOM),1,200) NOMBRE,\n"
                + "COALESCE(UBDEP,'') ID_DEPARTAMENTO, COALESCE(UBPRV,'') ID_PROVINCIA, COALESCE(UBDIS,'') ID_DISTRITO, COALESCE(DEDOMICIL,'') DE_DIRECCION, COALESCE(DEEMAIL,'') DE_CORREO,COALESCE(DETELEFO,'') TELEFONO \n"
    ...
    public String getCargoEmpleado(String coDep, String coEmp) {
         StringBuilder sql = new StringBuilder();
        String result = null;
        sql.append("SELECT IDOSGD.PK_SGD_DESCRIPCION_DE_CARGO_DOCUMENTO(?, ?) ");        
        try {
    ...
    public DatosInterBean DatosInter(String nuAnn, String nuEmi) {
        ...
        sql.append("SELECT (CASE WHEN LENGTH(A.NU_RUC_DES)>0 THEN A.NU_RUC_DES ELSE '0' END) AS NU_RUC_DES,"+
                ...
                " CASE WHEN LENGTH(A.REMI_NU_DNI_EMI)>0 THEN IDOSGD.PK_SGD_DESCRIPCION_ANI_SIMIL(a.REMI_NU_DNI_EMI) ELSE \n" +
                " CASE WHEN LENGTH(A.REMI_CO_OTR_ORI_EMI)>0 THEN IDOSGD.PK_SGD_DESCRIPCION_OTRO_ORIGEN(a.remi_co_otr_ori_emi) ELSE '' END END END) DE_NOM_DES,\n"+
                "(CASE WHEN LENGTH(A.DE_DEP_DES)>0 THEN DE_DEP_DES ELSE '' END) DE_DEP_DES,\n"+
                "(CASE WHEN LENGTH(A.DE_CAR_DES)>0 THEN DE_CAR_DES ELSE A.DE_CARGO END) DE_CAR_DES FROM IDOSGD.TDTV_DESTINOS A INNER JOIN IDOSGD.TDTV_REMITOS B ON A.NU_ANN=B.NU_ANN AND A.NU_EMI=B.NU_EMI \n" +
                "WHERE A.NU_ANN=?\n" +
                "AND A.NU_EMI=?\n" +
                "AND A.NU_DES=1 AND B.CO_TIP_DOC_ADM IN (SELECT CDOC_TIPDOC FROM IDOSGD.IOTDTX_TIPO_DOCUMENTO) ");
    ...
    public String insCargo(CargoFuncionBean cargo, String codusuario) {
        ...
        String genCoCargo = "";
        ...
        else
         {
            ...
            String seq = "SELECT LPAD((COALESCE(MAX(TO_NUMBER(CCAR_CO_CARGO,'999')), 0) + 1)::text, 3, '0') FROM IDOSGD.RHTM_CARGOS ";

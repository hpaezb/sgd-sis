...
public class AdmPermisoDaoImp extends SimpleJdbcDaoBase implements AdmPermisoDao {
    ...
    public List<PermisoBean> getPermisoList(String codDependencia) {
        ...
        sql.append("select s.cdes_user, p.co_use, p.co_dep, p.es_act, p.es_act_wf from TDTR_PERMISOS p inner join seg_usuarios1 s on p.co_use = s.cod_user where co_dep=? ");
    ...
    public String eliPermiso(String codDep) {
        ...
        sql.append("delete from TDTR_PERMISOS where co_dep=?");
        try {
    ...
    public String insertarPermiso(PermisoBean permisoDetalle) {
        String vReturn = "NO_OK";
        StringBuffer sql = new StringBuffer();
        sql.append("insert into TDTR_PERMISOS(co_use, co_dep, es_act) ");
        sql.append("values (?,?,?)");
        try {
            this.jdbcTemplate.update(sql.toString(), new Object[]{

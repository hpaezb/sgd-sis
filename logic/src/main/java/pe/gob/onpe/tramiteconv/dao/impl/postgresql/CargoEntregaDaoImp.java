...
public class CargoEntregaDaoImp extends SimpleJdbcDaoBase implements CargoEntregaDao {
    ...
    public List<CargoEntregaBean> getCargosEntrega(CargoEntregaBean cargo) {
        ...
        if (cargo.getEsFiltroFecha() != null
            ...
            if (vFechaIni != null && vFechaIni.trim().length() > 0
                ...
                sql.append(" AND A.FE_GUI_MP between TO_DATE(:pfechaIni,'dd/mm/yyyy') AND TO_DATE(:pfechaFin,'dd/mm/yyyy') + TIME '23:59:59'");
    ...
    public List<DocPedienteEntregaBean> getDocsPendienteEntrega(DocPedienteEntregaBean busqDoc) {
        ...
        sql.append(" TO_CHAR(A.FE_EMI,'DD/MM/YY') FE_EMI_CORTA,IDOSGD.PK_SGD_DESCRIPCION_DE_DOCUMENTO(A.CO_TIP_DOC_ADM) DE_TIP_DOC_ADM,");
        sql.append(" C.NU_DOC,IDOSGD.PK_SGD_DESCRIPCION_DE_DEPENDENCIA(B.CO_DEP_DES) DE_DEP_DES,");
        //sql.append(" DECODE (B.TI_DES,'01', IDOSGD.PK_SGD_DESCRIPCION_DE_NOM_EMP (B.CO_EMP_DES),'02', IDOSGD.PK_SGD_DESCRIPCION_DE_NOM_EMP (B.NU_RUC_DES), '03', IDOSGD.PK_SGD_DESCRIPCION_ANI_SIMIL (B.NU_DNI_DES), '04', IDOSGD.PK_SGD_DESCRIPCION_OTRO_ORIGEN (B.CO_OTR_ORI_DES)) DE_EMP_DES,");
        sql.append("CASE\n"
...
//            }
        ...
        if (pTipoBusqueda.equals("1"))
        {
            if (busqDoc.getNuCorEmi()!= null && busqDoc.getNuCorEmi().trim().length()>0){
               ...
               objectParam.put("pnuCorEmi", Integer.parseInt(busqDoc.getNuCorEmi()));
            }
    ...
    public String isDocPendienteEnGuiaMp(String pnuAnn,String pnuEmi,String pnuDes){
        ...
        try {
            vReturn = this.jdbcTemplate.queryForObject("select count(1) from IDOSGD.TDTD_GUIA_MP GD, IDOSGD.TDTC_GUIA_MP G\n" +
                      "where GD.NU_ANN_GUI=G.NU_ANN_GUI\n" +
                        ...
                        "AND G.ES_GUI_MP<> '9'\n" +
                        "AND G.ES_ELI = '0'", 
                    String.class, new Object[]{pnuAnn, pnuEmi, Integer.parseInt(pnuDes)});
        } catch (Exception e) {
            e.printStackTrace();
        }        
        return vReturn;
    ...
    public DocPedienteEntregaBean getDocPendienteEntrega(String pnuAnn,String pnuEmi,String pnuDes){
        ...
        sql.append("SELECT A.NU_ANN,A.NU_EMI,B.NU_DES,C.NU_EXPEDIENTE, IDOSGD.PK_SGD_DESCRIPCION_DE_DOCUMENTO(A.CO_TIP_DOC_ADM) DE_TIP_DOC_ADM,\n" +
                    "C.NU_DOC,TO_CHAR(A.FE_EMI,'DD/MM/YY') FE_EMI_CORTA, IDOSGD.PK_SGD_DESCRIPCION_DE_DEPENDENCIA(B.CO_DEP_DES) DE_DEP_DES,\n" +
                    ...
                    "AND B.NU_ANN=A.NU_ANN AND B.NU_EMI=A.NU_EMI AND B.NU_DES=CAST(? AS BIGINT)\n" +
    ...
    public String insGuiaMp(GuiaMesaPartesBean guia){
        ...
        sqlUpd.append("INSERT INTO IDOSGD.TDTC_GUIA_MP(\n"
                ...
                + "FE_USE_CRE)\n"
                + "VALUES(?,?,?,?,?,?,'0',to_timestamp(?,'DD/MM/YYYY HH24:MI'),?,CAST (? as bigint) ,'0',?, now(),?, now())");
    ...
    public String updGuiaMp(GuiaMesaPartesBean guia){
        ...
        sqlUpd.append("UPDATE IDOSGD.TDTC_GUIA_MP\n"
                + "SET FE_GUI_MP=to_timestamp(?,'dd/mm/yyyy hh24:mi:ss')\n"
                + ",DE_OBS=?\n"
                + ",FE_USE_MOD= now()\n"
                + ",CO_USE_MOD=?\n"
                + "WHERE\n"
    ...
    public String insDetGuiaMp(DetGuiaMesaPartesBean detGuia){
        ...
        sqlUpd.append("INSERT INTO IDOSGD.TDTD_GUIA_MP(\n"
                ...
                + "VALUES(?,?, CAST (? as bigint),?,?, CAST (? as bigint),'0','0',now(),?,?, now())");
    ...
    public String getNroGuiaCabecera(String pnuAnnGuia){
        String vReturn="-1";
        try {
            vReturn = this.jdbcTemplate.queryForObject("SELECT COALESCE(MAX(cast(NU_GUI as int)), 0) + 1\n" +
                                        "FROM IDOSGD.TDTC_GUIA_MP\n" +
                                        "WHERE NU_ANN_GUI=?", 
    ...
    public String getNroCorrelativoGuiaCabecera(String pnuAnnGuia,String pcoDepOri){
        String vReturn="-1";
        try {
            vReturn = this.jdbcTemplate.queryForObject("SELECT COALESCE(MAX(NU_COR_GUI), 0) + 1\n" +
                                        "FROM IDOSGD.TDTC_GUIA_MP\n" +
                                        "WHERE NU_ANN_GUI=?\n" +
                                        "and CO_DEP_ORI=?", 
    ...
    public String getNroCorrelativoDetGuiaCabecera(String pnuAnnGuia, String pnuGuia){
        String vReturn="-1";
        try {
            vReturn = this.jdbcTemplate.queryForObject("SELECT COALESCE(MAX(NU_COR), 0) + 1\n" +
                            "FROM IDOSGD.TDTD_GUIA_MP\n" +
                            "WHERE NU_ANN_GUI=?\n" +
                            "AND NU_GUI=?", 
    ...
    public List<DocPedienteEntregaBean> getDetalleGuiaMp(String pnuAnnGuia,String pnuGuia){
        ...
        "RR.NU_DOC,TO_CHAR(R.FE_EMI,'DD/MM/YY') FE_EMI_CORTA,IDOSGD.PK_SGD_DESCRIPCION_DE_DEPENDENCIA(D.CO_DEP_DES) DE_DEP_DES,\n" +
        ...
        "FROM IDOSGD.TDTC_GUIA_MP G, IDOSGD.TDTD_GUIA_MP GD, IDOSGD.TDTV_REMITOS R,IDOSGD.TDTV_DESTINOS D, IDOSGD.TDTX_REMITOS_RESUMEN RR\n" +
        "WHERE\n" +
        "G.NU_ANN_GUI = ?\n" +
        "AND G.NU_GUI = ?\n" +
        "AND GD.NU_ANN_GUI=?\n" +
    ...
    public List<CargoEntregaBean> getListaReporteBusqueda(CargoEntregaBean cargo) {
        ...
        prutaReporte.append("			WHEN '03' THEN COALESCE(IDOSGD.PK_SGD_DESCRIPCION_ANI_SIMIL(d.nu_dni_des), ");
        ...
        prutaReporte.append("FROM IDOSGD.TDTC_GUIA_MP G, ");
        prutaReporte.append("     IDOSGD.TDTD_GUIA_MP GD, ");
        prutaReporte.append("     IDOSGD.TDTV_DESTINOS D, ");
        prutaReporte.append("     IDOSGD.TDTV_REMITOS R, ");

...
public class OtroOrigenDaoImp extends SimpleJdbcDaoBase implements OtroOrigenDao {
    ...
    public List<OtroOrigenBean> getOtrosOrigenesList() {
        ...
        sql.append("IDOSGD.PK_SGD_DESCRIPCION_fu_get_distrito ( ub_dep, ub_pro, ub_dis) no_dis, es_activo,de_email, de_telefo FROM IDOSGD.tdtr_otro_origen ");
    ...
    public List<SiElementoBean> getTipoDocumentoList() {
        StringBuffer sql = new StringBuffer();
        sql.append("select CELE_DESELE,CELE_CODELE from IDOSGD.SI_ELEMENTO WHERE ");
        sql.append("CTAB_CODTAB='TIP_DOC_IDENT' AND CELE_CODELE IN ('01','02','03','04','05','06') ");
    ...
    public String insOtroOrigen(OtroOrigenBean otroOrigen) {
        ...
        sql.append(")values(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?) ");
        try {            
            String id = (String) this.jdbcTemplate.queryForObject("SELECT LPAD(CAST(COALESCE(MAX(cast(co_otr_ori as int)),0)+1 AS TEXT),10,'0') FROM IDOSGD.tdtr_otro_origen", String.class);
    ...
    public OtroOrigenBean getOtroOrigen(String codOrigen) {
        ...
        sql.append("IDOSGD.PK_SGD_DESCRIPCION_fu_get_departamento (ub_dep) no_dep, ");
        sql.append("IDOSGD.PK_SGD_DESCRIPCION_fu_get_provincia (ub_dep,ub_pro) no_pro, ");
        sql.append("IDOSGD.PK_SGD_DESCRIPCION_fu_get_distrito (ub_dep,ub_pro ,ub_dis) no_dis ");
        sql.append("FROM IDOSGD.tdtr_otro_origen ");

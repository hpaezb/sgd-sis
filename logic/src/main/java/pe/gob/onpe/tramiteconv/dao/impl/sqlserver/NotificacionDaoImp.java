...
public class NotificacionDaoImp extends SimpleJdbcDaoBase implements NotificacionDao{
    ...
    public DocumentoDatoBean getDatosDoc(String nuAnn, String nuEmi){
        ...
        sql.append("SELECT A.NU_EMI, \n"
                + "A.NU_ANN,\n"
                //+ "TO_CHAR(A.FE_EMI,'DD/MM/YYYY HH24:MI') FECHA_DOC, \n"
                +" (CONVERT(VARCHAR(10), A.FE_EMI, 103)  +' '+ convert(VARCHAR(8), A.FE_EMI, 14)) FECHA_DOC,"
                + "B.NU_DOC SIGLAS_DOC, \n"
                + "A.DE_ASU, \n"
                + "A.ES_DOC_EMI, \n"
   ...
   public String notificarVistoBueno(String nuEmi,String nuAnn){
        ...
        sql.append("select R.CEMP_APEPAT+' '+R.CEMP_APEMAT+' '+R.CEMP_DENOM as cempDenom ,R.CEMP_EMAIL as compName,\n" +
                    "'<b>'+tip.CDOC_DESDOC+' '+ISNULL(rt.NU_DOC_EMI,'')+'-'+(SELECT TOP 1 DE_PAR FROM IDOSGD.TDTR_PARAMETROS WHERE CO_PAR='DE_INSTITUCION'  )+'-'+rt.DE_DOC_SIG+'<br/>'+'FECHA:'+   CONVERT(varchar(20),rt.FE_EMI, 120) +'</b><br/><p ><b>ASUNTO:</b> '+rt.DE_ASU+'</p>' as accionBD "+
                    "from IDOSGD.TDTV_PERSONAL_VB T WITH (NOLOCK) \n" +
                    "inner join IDOSGD.tdtv_remitos rt  WITH (NOLOCK) ON rt.NU_ANN=T.NU_ANN AND rt.NU_EMI=T.NU_EMI\n" +
                    "inner join IDOSGD.si_mae_tipo_doc tip WITH (NOLOCK)  ON tip.CDOC_TIPDOC=rt.CO_TIP_DOC_ADM\n" +
                    "inner join IDOSGD.rhtm_per_empleados R  WITH (NOLOCK) ON T.CO_EMP_VB=R.CEMP_CODEMP\n" +
    ...
    StringBuffer sql = new StringBuffer();
        ...
        sql.append("select DISTINCT R.CEMP_APEPAT+' '+R.CEMP_APEMAT+' '+R.CEMP_DENOM as cempDenom ,R.CEMP_EMAIL as compName,\n" +
                    "'<b>EXP. '+(SELECT TOP 1 ISNULL(NU_EXPEDIENTE,'') FROM IDOSGD.TDTC_EXPEDIENTE WHERE NU_SEC_EXP=rt.NU_SEC_EXP  )+'<br/>'+ISNULL(tip.CDOC_DESDOC,'')+' '+ISNULL(rt.NU_DOC_EMI,'')+'-'+ (CASE WHEN rt.TI_EMI ='01' THEN  (SELECT TOP 1 DE_PAR FROM IDOSGD.TDTR_PARAMETROS WHERE CO_PAR='DE_INSTITUCION'  )  ELSE '' END ) +'-'+rt.DE_DOC_SIG+'<br/>'+'FECHA:'+ CONVERT(varchar(20),rt.FE_EMI, 120) +'</b><br/><p ><b>ASUNTO:</b> '+rt.DE_ASU+'</p>' as accionBD "+
                    "from IDOSGD.tdtv_destinos T WITH (NOLOCK) \n" +
                    "inner join IDOSGD.tdtv_remitos rt  WITH (NOLOCK) ON rt.NU_ANN=T.NU_ANN AND rt.NU_EMI=T.NU_EMI\n" +
                    "inner join IDOSGD.si_mae_tipo_doc tip  WITH (NOLOCK) ON tip.CDOC_TIPDOC=rt.CO_TIP_DOC_ADM\n" +
                    "inner join IDOSGD.rhtm_per_empleados R WITH (NOLOCK)  ON T.CO_EMP_DES=R.CEMP_CODEMP\n" +
                    "inner join IDOSGD.TDTX_CONFIG_EMP C  WITH (NOLOCK) ON R.CEMP_CODEMP=C.CO_EMP  \n" +
    ...
    public void RegisternotificarCorreo(String v_nom, String p_Para, String detalleDoc, String asunto, String cuerpo) {
      ...
      SimpleJdbcCall spNotificarCorreo = new SimpleJdbcCall(this.dataSource).withProcedureName("[IDOSGD].[PK_SGD_TRAMITE_CREA_NOTIFICACION_LOG_INSERTA]")
            .withoutProcedureColumnMetaDataAccess()
            .useInParameterNames("NOMBRES", "PARA","DETALLE","ASUNTO","CUERPO")
            .declareParameters(
                new SqlParameter("NOMBRES", Types.VARCHAR),
                new SqlParameter("PARA", Types.VARCHAR),
                new SqlParameter("DETALLE", Types.VARCHAR),
                new SqlParameter("ASUNTO", Types.VARCHAR),
                new SqlParameter("CUERPO", Types.VARCHAR));
                
        SqlParameterSource in = new MapSqlParameterSource()        
                .addValue("NOMBRES", v_nom)
                .addValue("PARA", p_Para)
                .addValue("DETALLE", detalleDoc)
                .addValue("ASUNTO", asunto)
                .addValue("CUERPO", cuerpo);
    ...
    public String getLsMesaVirtualNotificar(String nuEmi, String nuAnn, String detalleDoc) {
        ...
        sql.append("select DISTINCT 'Administrador' as cempDenom ,si.CELE_DESCOR as compName,\n" +
                    "'<b>EXP. '+(SELECT TOP 1 NU_EXPEDIENTE FROM IDOSGD.TDTC_EXPEDIENTE WHERE NU_SEC_EXP=rt.NU_SEC_EXP )+'<br/>'+tip.CDOC_DESDOC+' '+rt.NU_DOC_EMI+'-'+(SELECT TOP 1 DE_PAR FROM IDOSGD.TDTR_PARAMETROS WHERE CO_PAR='DE_INSTITUCION'  )+'-'+rt.DE_DOC_SIG+'<br/>'+'FECHA:'+CONVERT(NVARCHAR,rt.FE_EMI, 120)+'</b><br/><p ><b>ASUNTO:</b> '+rt.DE_ASU+'</p>' as accionBD "+
                    "from IDOSGD.tdtv_destinos T WITH (NOLOCK) \n" +
                    "inner join IDOSGD.tdtv_remitos rt  WITH (NOLOCK) ON rt.NU_ANN=T.NU_ANN AND rt.NU_EMI=T.NU_EMI\n" +
                    "inner join IDOSGD.si_mae_tipo_doc tip WITH (NOLOCK)  ON tip.CDOC_TIPDOC=rt.CO_TIP_DOC_ADM\n" +
                    "inner join IDOSGD.IOTDTC_DESPACHO io  WITH (NOLOCK) ON io.VNUMREGSTD=T.NU_EMI and io.VANIOREGSTD=T.NU_ANN \n" +
                    "left join IDOSGD.si_elemento si  WITH (NOLOCK) ON si.CTAB_CODTAB='NOTI_DESP_MV' \n" +
    ...
    public String getLsTupaNotificar(String nuEmi, String nuAnn, String detalleDoc) {
        ...
        List<EmpleadoBean> list= null;
        sql.append("select DISTINCT 'Administrador' as cempDenom ,si.CELE_DESCOR as compName,\n" +
                    "(select TOP 1 proc1.de_nombre from IDOSGD.tdtr_procesos_exp proc1 where proc1.co_proceso=si.CELE_DESELE )+'<br/>'+'<b>EXP. '+ex.NU_EXPEDIENTE+'<br/>'+tip.CDOC_DESDOC+' '+rt.NU_DOC_EMI+'-'+(SELECT TOP 1  DE_PAR FROM IDOSGD.TDTR_PARAMETROS WHERE CO_PAR='DE_INSTITUCION' )+'-'+rt.DE_DOC_SIG+'<br/>'+'FECHA:'+CONVERT(NVARCHAR,rt.FE_EMI,120)+'</b><br/><p ><b>ASUNTO:</b> '+rt.DE_ASU+'</p>' as accionBD "+

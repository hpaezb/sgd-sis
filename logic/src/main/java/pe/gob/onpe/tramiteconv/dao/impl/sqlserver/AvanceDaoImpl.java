...
public class AvanceDaoImpl extends SimpleJdbcDaoBase implements AvanceDao{
    ...
    public List<AvanceBean> getListAvance(AvanceBean buscarAvanzadaBean) {
        ...
        sql.append(" SELECT     nu_ann,    nu_emi, nu_des ,  es_ultimo,    de_avance,    fe_avance,    ti_avance,    es_eli,    co_user\n" +
                   " FROM   IDOSGD.tdtr_avance WHERE nu_ann=? and nu_emi=? and nu_des=? and ti_avance=? ORDER BY FE_AVANCE DESC ");        
    ...
    public String insertAvance(AvanceBean buscarAvanzadaBean) {
        ...
        sqlupda.append("update IDOSGD.TDTV_DESTINOS set DE_AVANCE = ?  where nu_ann+nu_emi+CAST(nu_des as nvarchar(10))=? "); 
        sqlIns.append("INSERT INTO IDOSGD.tdtr_avance (  nu_ann,  nu_emi,nu_des,   ti_avance,  es_ultimo,  de_avance,   fe_avance, es_eli,  co_user) \n" +
                       " VALUES ( ?,?,CAST(? as numeric),?,'1',?,getdate(),'0',?) "); 
        try{
